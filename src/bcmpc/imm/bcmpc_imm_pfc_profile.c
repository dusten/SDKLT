/*! \file bcmpc_imm_pfc_profile.c
 *
 * PC_PFC_PROFILEt IMM handler.
 */
/*
...
#define IMM_SID PC_PFC_PROFILEt

/*! The data structure for PC_PFC_PROFILEt entry. */
typedef struct pc_pfc_prof_entry_s {
    int key;
    bcmpc_pfc_prof_t data;
} pc_pfc_prof_entry_t;
 ...
 */
 ...
 /*!
  * \brief Initialize pc_pfc_prof_entry_t data buffer.
  *
  ...
  */
static void
pc_pfc_prof_entry_t_init(pc_pfc_prof_entry_t *entry)
{
    entry->key = BCMPC_INVALID_PROF_ID;
    bcmpc_pfc_prof_t_init(&entry->data);
}
 ...
 */
static int
pc_pfc_prof_entry_set(int unit, uint32_t fid, uint64_t fval,
                      pc_pfc_prof_entry_t *entry)
{
    bcmpc_pfc_prof_t *prof = &entry->data;

    SHR_FUNC_ENTER(unit);

    switch (fid) {
        case PC_PFC_PROFILEt_PC_PFC_PROFILE_IDf:
            entry->key = fval;
            break;
        case PC_PFC_PROFILEt_ENABLE_RXf:
            prof->rx_enable = fval;
            break;
        case PC_PFC_PROFILEt_ENABLE_TXf:
            prof->tx_enable = fval;
            break;
        case PC_PFC_PROFILEt_ENABLE_STATSf:
            prof->stats_enable = fval;
            break;
        case PC_PFC_PROFILEt_REFRESH_TIMERf:
            prof->refresh_timer = fval;
            break;
        case PC_PFC_PROFILEt_DEST_ADDRf:
            sal_memcpy(prof->da, &fval, sizeof(prof->da));
            break;
        case PC_PFC_PROFILEt_ETH_TYPEf:
            prof->eth_type = fval;
            break;
        case PC_PFC_PROFILEt_OPCODEf:
            prof->opcode = fval;
            break;
        case PC_PFC_PROFILEt_XOFF_TIMERf:
            prof->xoff_timer = fval;
            break;
        default:
 ...
 *
 * Parse the input LT fields and save the data to pc_pfc_prof_entry_t.
 *
 * \param [in] unit Unit number.
 ...
 * \retval SHR_E_FAIL Faild to parse the input fields.
 */
static int
pc_lt_pfc_prof_flds_parse(int unit,
                          ...
                          pc_pfc_prof_entry_t *entry)
{
    ...
    while (fld) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (pc_pfc_prof_entry_set(unit, fid, fval, entry));
                     ...
                     bcmltd_fields_t *output_fields)
{
    ...
    pc_pfc_prof_entry_t_init(&entry);
    ...
    switch (event) {
        case BCMIMM_ENTRY_INSERT:
            SHR_IF_ERR_VERBOSE_EXIT
                (pc_lt_pfc_prof_flds_parse(unit, key_flds, data_flds,
                                           &entry));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmpc_pfc_prof_internal_update(unit, entry.key,
                                                &entry.data));
            break;
        case BCMIMM_ENTRY_UPDATE:
            SHR_IF_ERR_VERBOSE_EXIT
                (pc_lt_pfc_prof_flds_parse(unit, key_flds, NULL,
                                           &entry));
            SHR_IF_ERR_VERBOSE_EXIT
            ...
            SHR_IF_ERR_VERBOSE_EXIT
                (pc_lt_pfc_prof_flds_parse(unit, NULL, data_flds,
                                           &entry));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmpc_pfc_prof_internal_update(unit, entry.key,
                                                &entry.data));
 ...
 */
static int
pc_pfc_profile_entry_parse(int unit, const bcmltd_fields_t *flds,
 ...
 * bcmpc_db_imm_schema_handler_t.
 *
 * Convert an integer profile id \c key and a bcmpc_pfc_prof_t \c data to
 ...
 */
static int
pc_pfc_profile_entry_fill(int unit,
                                 size_t key_size, void *key,
                                 size_t data_size, void *data,
                                 bcmltd_fields_t *flds)
{
    ...
    if (key) {
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PFC_PROFILEt_PC_PFC_PROFILE_IDf, 0,
             *prof_id, fcnt);
    }

    if (data) {
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PFC_PROFILEt_ENABLE_RXf, 0,
             prof->rx_enable, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PFC_PROFILEt_ENABLE_TXf, 0,
             prof->tx_enable, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PFC_PROFILEt_ENABLE_STATSf, 0,
             prof->stats_enable, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PFC_PROFILEt_REFRESH_TIMERf, 0,
             prof->refresh_timer, fcnt);
        BCMPC_LT_FIELD_DATA_ADD
            (flds, PC_PFC_PROFILEt_DEST_ADDRf, 0,
             &prof->da, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PFC_PROFILEt_ETH_TYPEf, 0,
             prof->eth_type, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PFC_PROFILEt_OPCODEf, 0,
             prof->opcode, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PFC_PROFILEt_XOFF_TIMERf, 0,
             prof->xoff_timer, fcnt);
    }
...
bcmpc_imm_pfc_profile_register(int unit)
{
    ...
    db_hdl.entry_parse = pc_pfc_profile_entry_parse;
    db_hdl.entry_fill = pc_pfc_profile_entry_fill;

    event_hdl.validate = NULL;
    event_hdl.stage = pc_pfc_profile_stage;
