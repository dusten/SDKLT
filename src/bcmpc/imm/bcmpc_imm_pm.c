...
#define IMM_SID PC_PMt

/*! The data structure for PC_PMt entry. */
typedef struct pc_pm_entry_s {
    int key;
    bcmpc_pm_mode_t data;
} pc_pm_entry_t;
 ...
 */
 ...
 /*!
  * \brief Initialize pc_pm_entry_t data buffer.
  *
  ...
  */
static void
pc_pm_entry_t_init(pc_pm_entry_t *entry)
{
    entry->key = -1;
    bcmpc_pm_mode_t_init(&entry->data);
}
 ...
 */
static int
pc_pm_entry_set(int unit, uint32_t fid, uint32_t fidx, uint64_t fval,
                pc_pm_entry_t *entry)
{
    bcmpc_pm_mode_t *pm_mode = &entry->data;

    SHR_FUNC_ENTER(unit);

    switch (fid) {
        case PC_PMt_PC_PM_IDf:
            entry->key = fval;
            break;
        case PC_PMt_PM_OPMODEf:
            if (fidx >= BCMPC_NUM_AGGR_CORES_PER_PM_MAX) {
                SHR_RETURN_VAL_EXIT(SHR_E_FAIL);
            }
            pm_mode->opmode[fidx] = fval;
            break;
        case PC_PMt_SPEED_MAXf:
            if (fidx >= BCMPC_NUM_PPORTS_PER_PM_MAX) {
                SHR_RETURN_VAL_EXIT(SHR_E_FAIL);
            }
            pm_mode->speed_max[fidx] = fval;
            break;
        case PC_PMt_LANE_MAPf:
            if (fidx >= BCMPC_NUM_PPORTS_PER_PM_MAX) {
                SHR_RETURN_VAL_EXIT(SHR_E_FAIL);
            }
            pm_mode->lane_map[fidx] = fval;
            break;
        default:
            SHR_RETURN_VAL_EXIT(SHR_E_FAIL);
    }
 ...
 *
 * Parse the input LT fields and save the data to pc_pm_entry_t.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
pc_lt_pm_fields_parse(int unit,
                      ...
                      pc_pm_entry_t *entry)
{
    ...
    while (fld) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (pc_pm_entry_set(unit, fid, fidx, fval, entry));
 ...
 */
static int
pc_lt_pm_update(int unit, pc_pm_entry_t *entry)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmpc_internal_pm_mode_set(unit, entry->key, &entry->data));
 ...
 */
static int
pc_lt_pm_delete(int unit, pc_pm_entry_t *entry)
{
    SHR_FUNC_ENTER(unit);

    SHR_IF_ERR_VERBOSE_EXIT
        (bcmpc_internal_pm_mode_delete(unit, entry->key));
 ...
 *
 * Convert bcmltd_fields_t array \c flds to an integer PM ID \c key and a
 * bcmpc_pm_mode_t \c data.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
pc_imm_pm_entry_parse(int unit, const bcmltd_fields_t *flds,
 ...
 * bcmpc_db_imm_schema_handler_t.
 *
 * Convert an integer PM ID \c key and a bcmpc_pm_mode_t \c data to
                     ...
                     bcmltd_fields_t *flds)
{
    size_t fcnt = 0;
    int idx, *pm_id = key;
    bcmpc_pm_mode_t *pm_mode = data;

    SHR_FUNC_ENTER(unit);

    if (key) {
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PMt_PC_PM_IDf, 0, *pm_id, fcnt);
    }

    if (data) {
        for (idx = 0; idx < BCMPC_NUM_AGGR_CORES_PER_PM_MAX; idx++) {
            BCMPC_LT_FIELD_VAL_ADD
                (flds, PC_PMt_PM_OPMODEf, idx, pm_mode->opmode[idx], fcnt);
        }

        for (idx = 0; idx < BCMPC_NUM_PPORTS_PER_PM_MAX; idx++) {
            BCMPC_LT_FIELD_VAL_ADD
                (flds, PC_PMt_SPEED_MAXf, idx, pm_mode->speed_max[idx], fcnt);
        }

        for (idx = 0; idx < BCMPC_NUM_PPORTS_PER_PM_MAX; idx++) {
            BCMPC_LT_FIELD_VAL_ADD
                (flds, PC_PMt_LANE_MAPf, idx, pm_mode->lane_map[idx], fcnt);
        }
    }
            ...
            bcmltd_fields_t *output_fields)
{
    ...
    pc_pm_entry_t_init(&entry);
    SHR_IF_ERR_VERBOSE_EXIT
        (pc_lt_pm_fields_parse(unit, key_flds, NULL, &entry));
    ...
    switch (event) {
        case BCMIMM_ENTRY_INSERT:
            SHR_IF_ERR_VERBOSE_EXIT
                (pc_lt_pm_fields_parse(unit, NULL, data_flds, &entry));
            SHR_IF_ERR_VERBOSE_EXIT
                (pc_lt_pm_update(unit, &entry));
            break;
...
bcmpc_imm_pm_register(int unit)
{
    ...
    db_hdl.entry_parse = pc_imm_pm_entry_parse;
    db_hdl.entry_fill = pc_imm_pm_entry_fill;

    event_hdl.validate = NULL;
    event_hdl.stage = pc_pm_stage;
