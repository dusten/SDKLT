/*! \file bcmpc_imm_pm_lane.c
 *
 * PC_PM_LANEt IMM handler.
 */
/*
...
#define IMM_SID PC_PM_LANEt

/*! The data structure for PC_PM_TX_LANE_PROFILEt entry. */
typedef struct pc_pm_lane_entry_s {
    bcmpc_pm_lane_t key;
    bcmpc_pm_lane_cfg_t data;
} pc_pm_lane_entry_t;
 ...
 */
 ...
 /*!
  * \brief Initialize pc_pm_lane_entry_t data buffer.
  *
  ...
  */
static void
pc_pm_lane_entry_t_init(pc_pm_lane_entry_t *entry)
{
    sal_memset(&entry->key, 0, sizeof(entry->key));
    bcmpc_pm_lane_cfg_t_init(&entry->data);
}
 ...
 */
static int
pc_pm_lane_entry_set(int unit, uint32_t fid, uint32_t fidx, uint64_t fval,
                     pc_pm_lane_entry_t *entry)
{
    bcmpc_pm_lane_t *pm_lane = &entry->key;
    bcmpc_pm_lane_cfg_t *lcfg = &entry->data;

    SHR_FUNC_ENTER(unit);

    switch (fid) {
        case PC_PM_LANEt_PC_PM_IDf:
            pm_lane->pm_id = fval;
            break;
        case PC_PM_LANEt_CORE_INDEXf:
            pm_lane->core_idx = fval;
            break;
        case PC_PM_LANEt_CORE_LANEf:
            pm_lane->core_lane = fval;
            break;
        case PC_PM_LANEt_PORT_OPMODEf:
            if (fidx >= BCMPC_NUM_OPMODES_PER_UNIT_MAX) {
                SHR_RETURN_VAL_EXIT(SHR_E_FAIL);
            }
            lcfg->port_opmode[fidx] = fval;
            break;
        case PC_PM_LANEt_PC_PM_TX_LANE_PROFILE_IDf:
            if (fidx >= BCMPC_NUM_OPMODES_PER_UNIT_MAX) {
                SHR_RETURN_VAL_EXIT(SHR_E_FAIL);
            }
            lcfg->tx_prof_id[fidx] = fval;
            break;
        case PC_PM_LANEt_PC_PM_RX_LANE_PROFILE_IDf:
            if (fidx >= BCMPC_NUM_OPMODES_PER_UNIT_MAX) {
                SHR_RETURN_VAL_EXIT(SHR_E_FAIL);
            }
            lcfg->rx_prof_id[fidx] = fval;
            break;
        default:
            SHR_RETURN_VAL_EXIT(SHR_E_FAIL);
    }
 ...
 *
 * Parse the input LT fields and save the data to pc_pm_lane_entry_t.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
pc_lt_pm_lane_flds_parse(int unit,
                         ...
                         pc_pm_lane_entry_t *entry)
{
    ...
    while (fld) {
        ...
        SHR_IF_ERR_VERBOSE_EXIT
            (pc_pm_lane_entry_set(unit, fid, fidx, fval, entry));
                 ...
                 bcmltd_fields_t *output_fields)
{
    ...
    switch (event) {
        case BCMIMM_ENTRY_INSERT:
            SHR_IF_ERR_VERBOSE_EXIT
                (pc_lt_pm_lane_flds_parse(unit, key_flds, data_flds, &entry));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmpc_pm_lane_internal_update(unit, &entry.key, &entry.data));
            break;
        case BCMIMM_ENTRY_UPDATE:
            SHR_IF_ERR_VERBOSE_EXIT
                (pc_lt_pm_lane_flds_parse(unit, key_flds, NULL, &entry));
            SHR_IF_ERR_EXIT
                (bcmpc_db_imm_entry_lookup(unit,
                                           IMM_SID,
                                           P(&entry.key), P(&entry.data)));
            SHR_IF_ERR_VERBOSE_EXIT
                (pc_lt_pm_lane_flds_parse(unit, NULL, data_flds, &entry));
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmpc_pm_lane_internal_update(unit, &entry.key, &entry.data));
            break;
 ...
 *
 * Convert bcmltd_fields_t array \c flds to a bcmpc_pm_lane_t \c key and a
 * bcmpc_pm_lane_cfg_t \c data.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
pc_pm_lane_entry_parse(int unit, const bcmltd_fields_t *flds,
 ...
 * bcmpc_db_imm_schema_handler_t.
 *
 * Convert a bcmpc_pm_lane_t \c key and a bcmpc_pm_lane_cfg_t \c data to
 ...
 */
static int
pc_pm_lane_entry_fill(int unit,
                      size_t key_size, void *key,
                      size_t data_size, void *data,
                      bcmltd_fields_t *flds)
{
    ...
    if (key) {
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PM_LANEt_PC_PM_IDf, 0, pm_lane->pm_id, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PM_LANEt_CORE_INDEXf, 0, pm_lane->core_idx, fcnt);
        BCMPC_LT_FIELD_VAL_ADD
            (flds, PC_PM_LANEt_CORE_LANEf, 0, pm_lane->core_lane, fcnt);
    }

    if (data) {
        for (idx = 0; idx < BCMPC_NUM_OPMODES_PER_UNIT_MAX; idx++) {
            BCMPC_LT_FIELD_VAL_ADD
                (flds, PC_PM_LANEt_PORT_OPMODEf, idx,
                 lcfg->port_opmode[idx], fcnt);
        }
        for (idx = 0; idx < BCMPC_NUM_OPMODES_PER_UNIT_MAX; idx++) {
            BCMPC_LT_FIELD_VAL_ADD
                (flds, PC_PM_LANEt_PC_PM_TX_LANE_PROFILE_IDf, idx,
                 lcfg->tx_prof_id[idx], fcnt);
        }
        for (idx = 0; idx < BCMPC_NUM_OPMODES_PER_UNIT_MAX; idx++) {
            BCMPC_LT_FIELD_VAL_ADD
                (flds, PC_PM_LANEt_PC_PM_RX_LANE_PROFILE_IDf, idx,
                 lcfg->rx_prof_id[idx], fcnt);
        }
    }
...
bcmpc_imm_pm_lane_register(int unit)
{
    ...
    db_hdl.entry_parse = pc_pm_lane_entry_parse;
    db_hdl.entry_fill = pc_pm_lane_entry_fill;

    event_hdl.validate = NULL;
    event_hdl.stage = pc_pm_lane_stage;
