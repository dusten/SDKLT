...
/*!
 * Memory rang accessing flag: USE_INTR_MODE
 *
 * Use interrupt mode if needed.
 */
#define USE_INTR_MODE         (1 << 0)

/*!
 * Memory rang accessing flag: USE_SINGLE_DATA
 *
 * Only need to prepare single entry data buffer.
 * For read, this can be used for test.
 * For write, this can be used for cleaning a memory.
 */
#define USE_SINGLE_DATA       (1 << 1)

/*!
 * Memory rang accessing flag: USE_SINGLE_ADDR
 *
 * Only access to a single entry.
 * For read, this can achieve HW FIFO read operation.
 * For write, this can achieve HW FIFO write operation.
 */
#define USE_SINGLE_ADDR       (1 << 2)

/*!
 * Memory rang accessing flag: USE_INVERS_ADDR
 *
 * Access from the highest entry to the lowest entry.
 */
#define USE_INVERS_ADDR       (1 << 3)
 ...
 * \param [in] flags Operation indicators.
 *
 * \return 0 if no errors, otherwise a negative value.
 */
extern int
