                     ...
                     uint32_t *data, size_t wsize)
{
    ...
    if (bcmbd_simhook_read) {
        ...
        if (LOG_CHECK_VERBOSE(BSL_LOG_MODULE)) {
            ...
            bcmbd_cmicm_dump_reg(pb, unit, adext, addr, data, wsize);
    ...
    rv = bcmbd_cmicm_schan_op(unit, &schan_msg, 2, 1 + wsize);
    if (SHR_FAILURE(rv)) {
                      ...
                      uint32_t *data, size_t wsize)
{
    ...
    rv = bcmbd_cmicm_schan_op(unit, &schan_msg, 2 + wsize, 0);
    if (SHR_FAILURE(rv)) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
                              "SOC reg write error: "
