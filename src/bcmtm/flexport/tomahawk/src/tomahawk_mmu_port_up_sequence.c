/*! \file tomahawk_mmu_port_up_sequence.c
 *
 * Tomahawk mmu port up sequence functions.
 */
/*
...
/*! @fn int soc_tomahawk_mmu_port_up_sequence(int unit,
 ...
 */
int
soc_tomahawk_flex_mmu_reconfigure_phase2(
    int unit, soc_port_schedule_state_t *port_schedule_state_t)
{
    ...
    soc_tomahawk_mmu_get_bst_mode(unit, &bst_mode);

    for (port=0; port<port_schedule_state_t->nport; port++) {
        if (port_schedule_state_t->resource[port].physical_port != BCMPC_INVALID_PPORT) {
            oversub_ratio =
            ...
            soc_tomahawk_mmu_reinit_ct_setting(
                unit, &port_schedule_state_t->resource[port],
            ...
            soc_tomahawk_mmu_clear_mtro_bucket_mems(
                unit, &port_schedule_state_t->resource[port]);
            ...
            soc_tomahawk_mmu_wred_clr(unit,
                                       &port_schedule_state_t->resource[port]);
            soc_tomahawk_mmu_thdi_setup(unit,
                                         &port_schedule_state_t->resource[port],
                                         lossy);
...
int
soc_tomahawk_flex_mmu_port_up(int                        unit,
                               soc_port_schedule_state_t *port_schedule_state_t)
{
    int port;
    uint64 temp64;
    for (port=0; port<port_schedule_state_t->nport; port++) {
        if (port_schedule_state_t->resource[port].physical_port != BCMPC_INVALID_PPORT) {
            COMPILER_64_SET(temp64, 0, TH_MMU_FLUSH_OFF);
            soc_tomahawk_mmu_vbs_port_flush(
                unit, &port_schedule_state_t->resource[port], temp64);
            soc_tomahawk_mmu_rqe_port_flush(
                unit, &port_schedule_state_t->resource[port], temp64);
            soc_tomahawk_mmu_mtro_port_flush(
                unit, &port_schedule_state_t->resource[port], temp64);
        }
    }
    return SOC_E_NONE;
