 ...
 * \param [in] sched_port_id Scheduler port ID.
        ...
        bcmtm_scheduler_cfg_t *scheduler_cfg)
{
    ...
    if (bcmtm_scheduler_hdlr[dev_type].port_set) {
        SHR_IF_ERR_EXIT
            (bcmtm_scheduler_hdlr[dev_type].port_set(unit, ltid, lport,
        ...
        bcmtm_scheduler_cfg_t *scheduler_cfg)
{
    ...
    bcmtm_field_populate(unit, &in, fid, 0, fval);
    ...
    /*
     * perform an imm lookup and check for oper status. write to hardware
     * only if input is valid.
     */
    SHR_IF_ERR_EXIT(bcmimm_entry_lookup(unit, ltid, &in, &out));

    fid = TM_SCHEDULER_PORT_PROFILEt_OPERATIONAL_STATEf;
    SHR_IF_ERR_EXIT
        (bcmtm_fval_get_from_field_array(unit, fid, *(out.field), &fval));

    /* Physical table update */
    if (fval == 0) {
        SHR_IF_ERR_EXIT
            (bcmtm_scheduler_port_pt_set(unit, lport, ltid, NULL, scheduler_cfg));
    }
exit:
    SHR_FUNC_EXIT();
}
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_scheduler_profile_get(unit, scheduler_cfg->profile_id, profile));

    /* check for profile is OK to configure to hardware. */
    SHR_IF_ERR_EXIT
        (bcmtm_scheduler_profile_validate(unit, -1, profile, &status));
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    switch(event_reason) {
        ...
        case BCMIMM_ENTRY_INSERT:
            ...
            SHR_IF_ERR_EXIT
                (bcmtm_scheduler_port_set(unit, ltid, key, &scheduler_cfg,
                                       output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (bcmtm_scheduler_port_reset(unit, ltid, key, &scheduler_cfg));
            break;
        default:
            break;
    }
...
bcmtm_scheduler_port_imm_register(int unit)
{
    ...
    SHR_IF_ERR_EXIT_EXCEPT_IF
        (bcmlrd_map_get(unit, TM_SCHEDULER_PORT_PROFILEt, &map), SHR_E_UNAVAIL);
