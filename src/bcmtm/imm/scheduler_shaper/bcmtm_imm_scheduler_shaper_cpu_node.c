        ...
        bcmtm_scheduler_cfg_t *scheduler_cfg)
{
    ...
    scheduler_cfg->parent_node = 0xff; /* set to invalid. */
    while(cur_data) {
        switch(cur_data->id) {
        ...
        bcmtm_scheduler_cfg_t *scheduler_cfg)
{
    ...
    if (bcmtm_shaper_hdlr[dev_type].cpu_node_set) {
        SHR_IF_ERR_EXIT
            (bcmtm_shaper_hdlr[dev_type].cpu_node_set(unit, sched_node_id,
                                    sched_level, ltid, shaper_cfg));
    }
    if (bcmtm_scheduler_hdlr[dev_type].cpu_node_set) {
        SHR_IF_ERR_EXIT
            (bcmtm_scheduler_hdlr[dev_type].cpu_node_set(unit, sched_node_id,
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    /* Parent node is applicable only for L1 sched level. */
    if ((sched_level == 0) && (scheduler_cfg->parent_node != 0xff)) {
...
bcmtm_scheduler_shaper_cpu_node_imm_register(int unit)
{
    ...
    SHR_IF_ERR_EXIT_EXCEPT_IF
        (bcmlrd_map_get(unit, TM_SCHEDULER_SHAPER_CPU_NODEt, &map), SHR_E_UNAVAIL);
