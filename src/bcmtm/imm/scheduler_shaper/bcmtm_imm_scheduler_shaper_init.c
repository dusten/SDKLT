...
bcmtm_imm_scheduler_shaper_cb_reg(int unit)
{
    ...
    SHR_IF_ERR_EXIT(bcmtm_shaper_port_imm_register(unit));

    /* TM node shaper imm callback. */
    SHR_IF_ERR_EXIT(bcmtm_shaper_node_imm_register(unit));

    /* TM node scheduler imm callback. */
    SHR_IF_ERR_EXIT(bcmtm_scheduler_node_imm_register(unit));

    /* TM scheduler profile imm callback */
    SHR_IF_ERR_EXIT(bcmtm_scheduler_profile_imm_register(unit));

    /* TM node scheduler port imm callback. */
    SHR_IF_ERR_EXIT(bcmtm_scheduler_port_imm_register(unit));

    /* TM cpu node scheduler shaper imm callback. */
    SHR_IF_ERR_EXIT(bcmtm_scheduler_shaper_cpu_node_imm_register(unit));
exit:
    SHR_FUNC_EXIT();
}
