 ...
 * Local definitions
 */
bcmtm_shaper_hdlr_t bcmtm_shaper_hdlr[BCMDRD_DEV_T_COUNT];
        ...
        int enable)
{
    ...
    if (bcmtm_shaper_hdlr[dev_type].port_set) {
        SHR_IF_ERR_EXIT
            (bcmtm_shaper_hdlr[dev_type].port_set(unit, lport,
                                                 ltid, shaper_cfg, enable));
    }
exit:
    SHR_FUNC_EXIT();
}
    ...
    bcmtm_shaper_cfg_t *shaper_cfg)
{
    ...
    for (count = 0; count < num_fid; count++) {
        def = 0;
        if (SHR_E_UNAVAIL ==
                bcmtm_field_default_value_get(unit, ltid, count, &def))
            continue;
        switch(count) {
            case TM_SHAPER_PORTt_SHAPING_MODEf:
                shaper_cfg->shaping_mode = def;
                break;
            case TM_SHAPER_PORTt_ETHERNET_AVf:
                shaper_cfg->ethernet_av = def;
                break;
            default:
                break;
        }
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_shaper_rate_to_bucket(unit, lport, shaper_cfg->shaping_mode,
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_shaper_bucket_to_rate(unit, shaper_cfg->shaping_mode,
                                     &(shaper_cfg->max_bucket_encode)));
    ...
    bcmtm_field_populate(unit, output_fields, fid, 0,
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    switch(event_reason) {
        case BCMIMM_ENTRY_UPDATE:
            ...
            in.count = 0;
            bcmtm_field_populate(unit, &in, key->id, 0, key->data);
            SHR_IF_ERR_EXIT
        ...
        case BCMIMM_ENTRY_INSERT:
            ...
            SHR_IF_ERR_EXIT
                (bcmtm_shaper_port_set(unit, ltid, key, &shaper_cfg,
                                       output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (bcmtm_shaper_port_reset(unit, ltid, key, &shaper_cfg));
            break;
        default:
            break;
    }
...
bcmtm_shaper_port_imm_register(int unit)
{
    ...
    SHR_IF_ERR_EXIT_EXCEPT_IF
        (bcmlrd_map_get(unit, TM_SHAPER_PORTt, &map), SHR_E_UNAVAIL);
