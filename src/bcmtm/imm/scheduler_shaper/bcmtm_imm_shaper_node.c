        ...
        bcmtm_shaper_cfg_t *shaper_cfg)
{
    ...
    if (bcmtm_shaper_hdlr[dev_type].node_set) {
        SHR_IF_ERR_EXIT
            (bcmtm_shaper_hdlr[dev_type].node_set(unit, lport, 0, sched_node_id,
                                                 ltid, shaper_cfg));
    }
exit:
    SHR_FUNC_EXIT();
        ...
        bcmtm_shaper_cfg_t *shaper_cfg)
{
   ...
   /* TM_SHAPER_NODEt_PORT_IDf */
    ...
    /* return if logical port to physical port mapping is not available. */
    ...
    SHR_IF_ERR_EXIT
        (bcmtm_shaper_node_pt_set(unit, lport, sched_node_id, ltid, shaper_cfg));
exit:
    SHR_FUNC_EXIT();
}
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    switch(event_reason) {
        ...
        case BCMIMM_ENTRY_INSERT:
            ...
            SHR_IF_ERR_EXIT
                (bcmtm_shaper_node_set(unit, ltid, key, &shaper_cfg,
                                       output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (bcmtm_shaper_node_reset(unit, ltid, key, &shaper_cfg));
            break;
        default:
            break;
    }
...
bcmtm_shaper_node_imm_register(int unit)
{
    ...
    SHR_IF_ERR_EXIT_EXCEPT_IF
        (bcmlrd_map_get(unit, TM_SHAPER_NODEt, &map), SHR_E_UNAVAIL);
