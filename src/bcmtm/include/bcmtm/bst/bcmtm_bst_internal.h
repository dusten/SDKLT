 ...
 */
enum
bcmtm_bst_control_lt_fields {
    /*! BST tracking mode. */
    BST_CONTROL_TRACKING_MODE,

    /*! BST tracking mode. */
    BST_CONTROL_CLEAR_ON_READ,

    /*! BST tracking mode. */
    BST_CONTROL_SNAPSHOT,

    /*! BST mode max count. */
    BST_CONTROL_MAX
};

typedef struct bcmtm_bst_drv_s {
    bcmdrd_sid_t *config_sid;
    bcmdrd_fid_t (*config_fid)[BST_CONTROL_MAX];
} bcmtm_bst_drv_t;

#define BCMTM_BST_DRV(unit) bcmtm_bst_pt_drv[unit]

#define BCMTM_BST_CONFIG_PT_SID(unit) BCMTM_BST_DRV(unit)->config_sid
#define BCMTM_BST_CONFIG_PT_FID(unit) BCMTM_BST_DRV(unit)->config_fid
