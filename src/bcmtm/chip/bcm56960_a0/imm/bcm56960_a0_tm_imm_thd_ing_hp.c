/*! \file bcm56960_a0_tm_imm_thd_ing_hp.c
 *
 * TM ingress headroom pool threshold configuration for 56960_a0.
 */
/*
 ...
 */
static int
bcm56960_a0_tm_ing_hp_reset(int unit,
        bcmltd_sid_t ltid,
        const bcmltd_field_t *key)
{
    ...
    /* TM_THD_ING_HEADROOM_POOLt_TM_THD_HEADROOM_POOL_IDf */
    ltfid = TM_THD_ING_HEADROOM_POOLt_TM_THD_HEADROOM_POOL_IDf;
    SHR_IF_ERR_EXIT
        (bcmtm_fval_get_from_field_array(unit, ltfid, key, (uint64_t *)&hp_id));

    ltmbuf = 0;

    /* TM_THD_ING_HEADROOM_POOLt_LIMIT_CELLSf */
    BCMTM_PT_DYN_INFO(pt_dyn_info, hp_id, 0);
    for (idx = 0; idx < 4; idx++) {
        SHR_IF_ERR_EXIT
 ...
 */
static int
bcm56960_a0_tm_ing_hp_set(int unit,
        bcmltd_sid_t ltid,
        const bcmltd_field_t *key,
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    while(cur_data) {
        ltmbuf = 0;
        switch(cur_data->id) {
            case TM_THD_ING_HEADROOM_POOLt_LIMIT_CELLSf:
                BCMTM_PT_DYN_INFO(pt_dyn_info, hp_id, 0);
                fval = cur_data->data;
                SHR_IF_ERR_EXIT
                    (bcmtm_field_set(unit, sid[0], fid, &ltmbuf, &fval));
                for (idx = 0; idx < 4; idx++) {
                    SHR_IF_ERR_EXIT
                        (bcmtm_pt_indexed_write(unit, sid[idx], ltid,
                                                (void *)&pt_dyn_info, &ltmbuf));
 ...
 */
static int
bcm56960_a0_tm_ing_hp_stage(int unit,
        bcmltd_sid_t ltid,
        uint32_t trans_id,
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    switch(event_reason) {
        ...
        case BCMIMM_ENTRY_INSERT:
            ...
            SHR_IF_ERR_EXIT
                (bcm56960_a0_tm_ing_hp_set(unit, ltid, key,
                                                  data, output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (bcm56960_a0_tm_ing_hp_reset(unit, ltid, key));
            break;
        default:
            break;
    }
...
{
    /*! Inmemory callback functions for TM_THD_ING_HEADROOM_POOLt. */
    bcmimm_lt_cb_t bcmtm_ing_hp_imm_cb = {
        /*! Staging function. */
        .stage = bcm56960_a0_tm_ing_hp_stage,
    };
    int rv;
    const bcmlrd_map_t *map = NULL;

    SHR_FUNC_ENTER(unit);
    /*! Registers callback functions for ingress headroom pool. */
    rv = bcmlrd_map_get(unit, TM_THD_ING_HEADROOM_POOLt, &map);
    if (SHR_SUCCESS(rv) && map) {
        SHR_IF_ERR_EXIT(bcmimm_lt_event_reg(unit,
                                            TM_THD_ING_HEADROOM_POOLt,
                                            &bcmtm_ing_hp_imm_cb, NULL));
    }
exit:
    SHR_FUNC_EXIT();
}
