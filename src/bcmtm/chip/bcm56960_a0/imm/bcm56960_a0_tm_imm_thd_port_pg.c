/*! \file bcm56960_a0_tm_imm_thd_port_pg.c
 *
 ...
 */
static int
bcm56960_a0_tm_port_pg_reset(int unit,
        bcmltd_sid_t ltid,
        const bcmltd_field_t *key)
{
    ...
    /* TM_THD_PORT_PRI_GRPt_PORT_IDf */
    ltfid = TM_THD_PORT_PRI_GRPt_PORT_IDf;
    SHR_IF_ERR_EXIT
        (bcmtm_fval_get_from_field_array(unit, ltfid, key, &key_val));
    lport = (bcmtm_lport_t)key_val;

    /* TM_THD_PORT_PRI_GRPt_TM_THD_PORT_PRI_GRP_IDf */
    ltfid = TM_THD_PORT_PRI_GRPt_TM_THD_PORT_PRI_GRP_IDf;
    SHR_IF_ERR_EXIT
        (bcmtm_fval_get_from_field_array(unit, ltfid, key, &key_val));
    pg_id = (uint8_t)key_val;
    ...
    sid = THDI_PORT_PG_CONFIGm;
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_read(unit, sid, ltid,
                               (void *)&pt_dyn_info, ltmbuf_arr));
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, sid, PG_MIN_LIMITf, ltmbuf_arr, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, sid, PG_SHARED_DYNAMICf, ltmbuf_arr, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, sid, PG_SHARED_LIMITf, ltmbuf_arr, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, sid, PG_RESET_OFFSETf, ltmbuf_arr, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, sid, PG_RESET_FLOORf, ltmbuf_arr, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, sid, PG_GBL_HDRM_ENf, ltmbuf_arr, &fval));
    SHR_IF_ERR_EXIT
        (bcmtm_field_set(unit, sid, PG_HDRM_LIMITf, ltmbuf_arr, &fval));
    SHR_IF_ERR_EXIT
 ...
 */
static int
bcm56960_a0_tm_port_pg_set(int unit,
        bcmltd_sid_t ltid,
        const bcmltd_field_t *key,
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    int mport, pipe = 0, opcode = 1, index;
    uint64_t key_val;
    ...
    opcode = 0;
op_stage:
    ltfid = TM_THD_PORT_PRI_GRPt_OPERATIONAL_STATEf;
...
/*!
 * \brief TM_THD_PORT_PRI_GRPt callback function for staging.
 *
 * Handle TM_THD_PORT_PRI_GRP IMM table change events.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
bcm56960_a0_tm_port_pg_stage(int unit,
        bcmltd_sid_t ltid,
        uint32_t trans_id,
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    switch(event_reason) {
        ...
        case BCMIMM_ENTRY_INSERT:
            ...
            SHR_IF_ERR_EXIT
                (bcm56960_a0_tm_port_pg_set(unit, ltid, key,
                                                  data, output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (bcm56960_a0_tm_port_pg_reset(unit, ltid, key));
            break;
        default:
            break;
    }
...
{
    /*! Inmemory callback functions for TM_THD_PORT_PRI_GRPt logical table. */
    bcmimm_lt_cb_t bcmtm_port_pg_imm_cb = {
        /*! Staging function. */
        .stage = bcm56960_a0_tm_port_pg_stage,
    };
    int rv;
    const bcmlrd_map_t *map;

    SHR_FUNC_ENTER(unit);
    /*! Registers callback functions for port PG thresholds. */
    rv = bcmlrd_map_get(unit, TM_THD_PORT_PRI_GRPt, &map);
    if (SHR_SUCCESS(rv) && map) {
        SHR_IF_ERR_EXIT(bcmimm_lt_event_reg(unit,
                                            TM_THD_PORT_PRI_GRPt,
                                            &bcmtm_port_pg_imm_cb, NULL));
    }
exit:
    SHR_FUNC_EXIT();
}
