/*! \file bcm56960_a0_tm_imm_thd_egr_uc_portsp.c
 *
 * TM egress unicast port service pool threshold configuration for 56960_a0.
 */
/*
 ...
 */
/*!
 * \brief  Resets the physical table.
 *
 ...
 */
static int
bcm56960_a0_tm_egr_uc_portsp_reset(int unit,
        bcmltd_sid_t ltid,
        const bcmltd_field_t *key)
{
    ...
    uint8_t sp_id;
    int mport, pipe, index;
    uint32_t ltmbuf_arr[3] = {0}, fval = 0;
    uint64_t val;

    SHR_FUNC_ENTER(unit);

    /* TM_THD_EGR_UC_PORT_SERVICE_POOLt_PORT_IDf */
    ltfid = TM_THD_EGR_UC_PORT_SERVICE_POOLt_PORT_IDf;
    SHR_IF_ERR_EXIT
        (bcmtm_fval_get_from_field_array(unit, ltfid, key, &val));
    lport = (bcmtm_lport_t)val;

    /* TM_THD_EGR_UC_PORT_SERVICE_POOLt_TM_THD_SERVICE_POOL_IDf */
    ltfid = TM_THD_EGR_UC_PORT_SERVICE_POOLt_TM_THD_SERVICE_POOL_IDf;
    SHR_IF_ERR_EXIT
        (bcmtm_fval_get_from_field_array(unit, ltfid, key, &val));
    sp_id = (uint8_t)val;
    ...
    fid = SHARED_LIMIT_ENABLEf;
    index = mport + sp_id * 34;
    ...
    index = mport * 4 + sp_id; /* 4 service pools */
    BCMTM_PT_DYN_INFO(pt_dyn_info, index, pipe);
    SHR_IF_ERR_EXIT
        (bcmtm_pt_indexed_write(unit, sid, ltid,
                                (void *)&pt_dyn_info, ltmbuf_arr));
 ...
 */
static int
bcm56960_a0_tm_egr_uc_portsp_set(int unit,
        bcmltd_sid_t ltid,
        const bcmltd_field_t *key,
        ...
        bcmltd_fields_t *output_fields)
{
     ...
     * Returns error if local port to mmu_local port information is not
     * available. Updates the operational state.
     */
    if ((SHR_FAILURE(bcmtm_port_mmu_local_port_get(unit, lport, &mport)))||
        (SHR_FAILURE(bcmtm_port_pipe_get(unit, lport, &pipe)))) {
        goto op_stage;
    ...
    while(cur_data) {
        switch(cur_data->id) {
            case TM_THD_EGR_UC_PORT_SERVICE_POOLt_USE_SHARED_LIMITSf:
                sid = MMU_THDM_DB_PORTSP_CONFIGm;
                fid = SHARED_LIMIT_ENABLEf;
                index = mport + sp_id * 34;
                BCMTM_PT_DYN_INFO(pt_dyn_info, index, pipe);
            ...
            case TM_THD_EGR_UC_PORT_SERVICE_POOLt_SHARED_LIMIT_CELLSf:
                sid = MMU_THDU_CONFIG_PORTm;
                fid = SHARED_LIMITf;
                index = mport + sp_id * 4; /* 4 service pools */
                BCMTM_PT_DYN_INFO(pt_dyn_info, index, pipe);
                fval = cur_data->data;
                break;
    ...
    opcode = 0;
op_stage:
    ltfid = TM_THD_EGR_UC_PORT_SERVICE_POOLt_OPERATIONAL_STATEf;
...
/*!
 * \brief TM_THD_EGR_UC_PORT_SERVICE_POOL callback function for staging.
 *
 * Handle TM_THD_EGR_UC_PORT_SERVICE_POOL IMM table change events.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
bcm56960_a0_tm_egr_uc_portsp_stage(int unit,
        bcmltd_sid_t ltid,
        uint32_t trans_id,
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    switch(event_reason) {
        ...
        case BCMIMM_ENTRY_INSERT:
            ...
            SHR_IF_ERR_EXIT
                (bcm56960_a0_tm_egr_uc_portsp_set(unit, ltid, key,
                                                  data, output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (bcm56960_a0_tm_egr_uc_portsp_reset(unit, ltid, key));
            break;
        default:
            break;
    }
...
{
    /*! Inmemory callback functions for TM_THD_EGR_UC_PORT_SERVICE_POOLt . */
    bcmimm_lt_cb_t bcmtm_egr_uc_portsp_imm_cb = {
        /*! Staging function. */
        .stage = bcm56960_a0_tm_egr_uc_portsp_stage,
    };
    int rv;
    const bcmlrd_map_t *map = NULL;

    SHR_FUNC_ENTER(unit);
    /*! Registers callback functions for egress unicast port service pool. */
    rv = bcmlrd_map_get(unit, TM_THD_EGR_UC_PORT_SERVICE_POOLt, &map);
    if (SHR_SUCCESS(rv) && map) {
        SHR_IF_ERR_EXIT(bcmimm_lt_event_reg(unit,
                                            TM_THD_EGR_UC_PORT_SERVICE_POOLt,
                                            &bcmtm_egr_uc_portsp_imm_cb, NULL));
    }
exit:
    SHR_FUNC_EXIT();
}
