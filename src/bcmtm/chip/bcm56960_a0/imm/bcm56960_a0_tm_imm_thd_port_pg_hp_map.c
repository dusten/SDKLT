/*! \file bcm56960_a0_tm_imm_thd_port_pg_hp_map.c
 *
 ...
 */
static int
bcm56960_a0_tm_port_pg_hp_map_reset(int unit,
        bcmltd_sid_t ltid,
        const bcmltd_field_t *key)
{
    bcmdrd_sid_t sid = THDI_HDRM_PORT_PG_HPIDr;
    bcmdrd_fid_t fid[] = { PG0_HPIDf, PG1_HPIDf, PG2_HPIDf, PG3_HPIDf,
    ...
    /* TM_THD_PORT_PRI_GRP_TO_HEADROOM_POOLt_PORT_IDf */
    ltfid = TM_THD_PORT_PRI_GRP_TO_HEADROOM_POOLt_PORT_IDf;
    SHR_IF_ERR_EXIT
        (bcmtm_fval_get_from_field_array(unit, ltfid, key, &key_val));
    lport = (bcmtm_lport_t)key_val;

    /* TM_THD_PORT_PRI_GRP_TO_HEADROOM_POOLt_TM_THD_PORT_PRI_GRP_IDf */
    ltfid = TM_THD_PORT_PRI_GRP_TO_HEADROOM_POOLt_TM_THD_PORT_PRI_GRP_IDf;
    SHR_IF_ERR_EXIT
        (bcmtm_fval_get_from_field_array(unit, ltfid, key, &key_val));
 ...
 */
static int
bcm56960_a0_tm_port_pg_hp_map_set(int unit,
        bcmltd_sid_t ltid,
        const bcmltd_field_t *key,
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    if (cur_data && (cur_data->id ==
            TM_THD_PORT_PRI_GRP_TO_HEADROOM_POOLt_TM_THD_HEADROOM_POOL_IDf)) {
        fval = cur_data->data;
        BCMTM_PT_DYN_INFO(pt_dyn_info, 0, mport);
        SHR_IF_ERR_EXIT
            (bcmtm_pt_indexed_read(unit, sid, ltid,
    ...
    opcode = 0;
op_stage:
    ltfid = TM_THD_PORT_PRI_GRP_TO_HEADROOM_POOLt_OPERATIONAL_STATEf;
 ...
 */
static int
bcm56960_a0_tm_port_pg_hp_map_stage(int unit,
        bcmltd_sid_t ltid,
        uint32_t trans_id,
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    switch(event_reason) {
        ...
        case BCMIMM_ENTRY_INSERT:
            ...
            SHR_IF_ERR_EXIT
                (bcm56960_a0_tm_port_pg_hp_map_set(unit, ltid, key,
                                                  data, output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (bcm56960_a0_tm_port_pg_hp_map_reset(unit, ltid, key));
            break;
        default:
            break;
    }
...
bcm56960_a0_tm_port_pg_hp_map_imm_register(int unit)
{
     ...
     * TM_THD_PORT_PRI_GRP_TO_HEADROOM_POOLt table.
     */
    bcmimm_lt_cb_t bcmtm_port_pg_hp_map_imm_cb = {
        /*! Staging function. */
        .stage = bcm56960_a0_tm_port_pg_hp_map_stage,
    };
    int rv;
    const bcmlrd_map_t *map = NULL;

    SHR_FUNC_ENTER(unit);
    /*! Registers callback functions for mapping port PG to headroom pools. */
    rv = bcmlrd_map_get(unit, TM_THD_PORT_PRI_GRP_TO_HEADROOM_POOLt, &map);
    if (SHR_SUCCESS(rv) && map) {
        SHR_IF_ERR_EXIT(bcmimm_lt_event_reg(unit,
                                            TM_THD_PORT_PRI_GRP_TO_HEADROOM_POOLt,
                                            &bcmtm_port_pg_hp_map_imm_cb, NULL));
    }
exit:
    SHR_FUNC_EXIT();
}
