/*! \file bcm56960_a0_tm_imm_thd_egr_mc_portsp.c
 *
 * TM egress multicast port service pool threshold configuration for 56960_a0.
 */
/*
 ...
 */
static int
bcm56960_a0_tm_egr_mc_portsp_reset(int unit,
        bcmltd_sid_t ltid,
        const bcmltd_field_t *key)
{
    ...
    uint32_t ltmbuf_arr[3] = {0};
    uint64_t key_val;

    SHR_FUNC_ENTER(unit);

    sid = MMU_THDM_DB_PORTSP_CONFIGm;
    /* TM_THD_EGR_MC_PORT_SERVICE_POOLt_PORT_IDf */
    ltfid = TM_THD_EGR_MC_PORT_SERVICE_POOLt_PORT_IDf;

    SHR_IF_ERR_EXIT
        (bcmtm_fval_get_from_field_array(unit, ltfid, key, &key_val));
    lport = (bcmtm_lport_t)key_val;

    /* TM_THD_EGR_MC_PORT_SERVICE_POOLt_TM_THD_SERVICE_POOL_IDf */
    ltfid = TM_THD_EGR_MC_PORT_SERVICE_POOLt_TM_THD_SERVICE_POOL_IDf;
    SHR_IF_ERR_EXIT
        (bcmtm_fval_get_from_field_array(unit, ltfid, key, &key_val));
    sp_id = (uint8_t)key_val;
 ...
 */
static int
bcm56960_a0_tm_egr_mc_portsp_set(int unit,
        bcmltd_sid_t ltid,
        const bcmltd_field_t *key,
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    while(cur_data) {
        switch(cur_data->id) {
            case TM_THD_EGR_MC_PORT_SERVICE_POOLt_USE_SHARED_LIMITSf:
    ...
    opcode = 0;
op_stage:
    ltfid = TM_THD_EGR_MC_PORT_SERVICE_POOLt_OPERATIONAL_STATEf;
...
/*!
 * \brief TM_THD_EGR_MC_PORT_SERVICE_POOL callback function for staging.
 *
 * Handle TM_THD_EGR_MC_PORT_SERVICE_POOL IMM table change events.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
bcm56960_a0_tm_egr_mc_portsp_stage(int unit,
        bcmltd_sid_t ltid,
        uint32_t trans_id,
        ...
        bcmltd_fields_t *output_fields)
{
    ...
    switch(event_reason) {
        ...
        case BCMIMM_ENTRY_INSERT:
            ...
            SHR_IF_ERR_EXIT
                (bcm56960_a0_tm_egr_mc_portsp_set(unit, ltid, key,
                                                  data, output_fields));
            break;
        case BCMIMM_ENTRY_DELETE:
            SHR_IF_ERR_EXIT
                (bcm56960_a0_tm_egr_mc_portsp_reset(unit, ltid, key));
            break;
        default:
            break;
    }
...
{
    /*! Inmemory callback functions for TM_THD_EGR_MC_PORT_SERVICE_POOL table. */
    bcmimm_lt_cb_t bcmtm_egr_mc_portsp_imm_cb = {
        /*! Staging function. */
        .stage = bcm56960_a0_tm_egr_mc_portsp_stage,
    };
    ...
    /*! Registers callback functions for egress multicast port service pool. */
    rv = bcmlrd_map_get(unit, TM_THD_EGR_MC_PORT_SERVICE_POOLt, &map);
    if (SHR_SUCCESS(rv) && map) {
        SHR_IF_ERR_EXIT(bcmimm_lt_event_reg(unit,
                                            TM_THD_EGR_MC_PORT_SERVICE_POOLt,
                                            &bcmtm_egr_mc_portsp_imm_cb, NULL));
    }
exit:
    SHR_FUNC_EXIT();
}
