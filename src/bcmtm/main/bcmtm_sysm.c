...
{
    /*
     * Since bcmtm_pc_drv_tm_config_set will be called from bcmtm_attach, we
                    ...
                    uint32_t *blocking_component)
{
    ...
    if (tm_drv && tm_drv->dev_init) {
        if (SHR_FAILURE(tm_drv->dev_init(unit))) {
            SHR_RETURN_VAL_EXIT(SHR_SYSM_RV_ERROR);
        }
    }
...
static shr_sysm_rv_t
tm_sysm_pre_config(shr_sysm_categories_t instance_category,
                   int unit,
                   void *comp_data,
                   bool warm,
                   uint32_t *blocking_component)
                 ...
                 uint32_t *blocking_component)
{
    ...
    if (SHR_FAILURE(bcmtm_detach(unit))) {
        return SHR_SYSM_RV_ERROR;
    }
