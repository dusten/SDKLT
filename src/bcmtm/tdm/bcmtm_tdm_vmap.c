...
bcmtm_tdm_vmap_sister_chk(tdm_mod_t *tdm, tdm_vmap_t *vmap)
{
    ...
    if (cfg_space_sister > 0) {
        ...
        vmap_len = vmap->len;
        cal_tmp = (int*)
                TDM_ALLOC(vmap_len * sizeof(int), "cal_tmp");
                        ...
                        int len_max)
{
    ...
    /* Allocate mem. */
    tbl = (int **) TDM_ALLOC(wid_max * sizeof(int *), "vmap_tbl_l1");
    for (i = 0; i < wid_max; i++) {
        tbl[i] = (int *) TDM_ALLOC(len_max * sizeof(int), "vmap_tbl_l2");
    }
    r_a_arr = (int **) TDM_ALLOC(wid_max * sizeof(int *), "r_a_arr_l1");
    for (i = 0; i < wid_max; i++) {
...
bcmtm_tdm_pmlist_init(tdm_mod_t *tdm, tdm_vmap_pmlist_t *pmlist, int size)
{
    ...
    if (pmlist != NULL && size > 0) {
        ...
        pms = (tdm_vmap_pm_t *) TDM_ALLOC(size * sizeof(tdm_vmap_pm_t), "pms");
        if (pms != NULL) {
