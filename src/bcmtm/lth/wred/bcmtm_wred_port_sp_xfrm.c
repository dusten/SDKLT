/*! \file bcmtm_wred_port_sp_xfrm.c
 *
 * APIs for wred port service pool transform functions.
 */
/*
...
#include <bcmtm/wred/bcmtm_wred_port_sp_idx.h>
#include <bcmtm/wred/bcmtm_wred_port_sp_field.h>

/*******************************************************************************
 * Public functions
 */
int
bcmtm_wred_port_sp_idx_transform(int unit,
                                ...
                                const bcmltd_transform_arg_t *arg)
{
    ...
    SHR_IF_ERR_EXIT(
        bcmtm_port_mmu_local_port_get(unit, in->field[0]->data, &mmu_local));
    ...
    out->field[out->count]->data =
        arg->value[0] + (mmu_local * arg->value[1]) + in->field[1]->data;
...
int
bcmtm_wred_port_sp_field_transform(int unit,
                                 const bcmltd_fields_t *in,
                                 bcmltd_fields_t *out,
                                 const bcmltd_transform_arg_t *arg)
{
    ...
    while (count  < in->count) {
        switch(in->field[count]->id) {
            case TM_WRED_PORT_SERVICE_POOLt_DRAIN_TRACKf:
                id = arg->rfield[0];
                break;
            ...
            case TM_WRED_PORT_SERVICE_POOLt_WREDf:
                id = arg->rfield[2];
                break;
            case TM_WRED_PORT_SERVICE_POOLt_TM_WRED_TIMER_PROFILE_IDf:
                id = arg->rfield[3];
                break;
...
int
bcmtm_wred_port_sp_field_rev_transform(int unit,
                                 const bcmltd_fields_t *in,
                                 bcmltd_fields_t *out,
                                 const bcmltd_transform_arg_t *arg)
{
