/*! \file bcmecmp_lth_overlay_group.c
 *
...
#include <bcmecmp/bcmecmp_lth_overlay_group.h>
 ...
 */
/*!
 * \brief ECMP_OVERLAY logical table entry fields print function.
 *
 * Debug ECMP_OVERLAY logical table entry fields and values print function.
 *
 ...
 */
static int
bcmecmp_olay_ltd_fields_print(int unit,
...
/*!
 * \brief ECMP_OVERLAY logical table entry logical fields parse routine.
 *
 * Parse ECMP_OVERLAY logical table entry logical fields and store the data in
 * lt_entry structure.
 *
 * \param [in] unit Unit number.
 ...
 */
static int
bcmecmp_olay_lt_fields_parse(int unit,
    ...
    for (i = 0; i < in->count; i++) {
        ...
        switch (fid) {
            ...
            case ECMP_OVERLAYt_ECMP_UNDERLAY_IDf:
                ...
                lt_entry->uecmp_id[in->field[i]->idx] = (int) fval;
                                   ...
                                   const bcmltd_generic_arg_t *arg)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lth_overlay_group_validate.\n")));
    ...
    LOG_DEBUG(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            "rv=%d"),
                bcmecmp_olay_ltd_fields_print(unit, in, 1)));
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmecmp_olay_lt_fields_parse(unit, in, arg->sid, lt_entry));
     ...
     * stored value.
     */
    if ((BCMLT_OPCODE_UPDATE == opcode)
        && !BCMECMP_LT_FIELD_GET
                (lt_entry->fbmp, BCMECMP_LT_FIELD_ECMP_NHOP)
        && BCMECMP_TBL_REF_CNT
                (BCMECMP_TBL_PTR(unit, group, lt_entry->grp_type),
                                 lt_entry->ecmp_id)
        && (BCMECMP_GRP_LT_SID(unit, lt_entry->ecmp_id) == arg->sid)) {
        lt_entry->ecmp_nhop = BCMECMP_GRP_ECMP_NHOP(unit, lt_entry->ecmp_id);
    }
                                 ...
                                 const bcmltd_generic_arg_t *arg)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lth_overlay_group_insert.\n")));
 ...
 *
 * Lookup an entry in ECMP_OVERLAY logical table and populate entry logical
                                 ...
                                 const bcmltd_generic_arg_t *arg)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lth_overlay_group_lookup.\n")));
    ...
    if (SHR_E_NONE == rv) {
        ...
        LOG_DEBUG(BSL_LOG_MODULE,
                    (BSL_META_U(unit,
                                "rv=%d"),
                    bcmecmp_olay_ltd_fields_print(unit, out, 0)));
    } else {
                                 ...
                                 const bcmltd_generic_arg_t *arg)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lth_overlay_group_delete.\n")));
                                 ...
                                 const bcmltd_generic_arg_t *arg)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lth_overlay_group_update.\n")));
...
/*!
 * \brief ECMP_OVERLAY logical table get first entry.
 *
                                ...
                                const bcmltd_generic_arg_t *arg)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lth_overlay_group_first: LT_SID=%d.\n"),
              arg->sid));
    ...
    /*
     * Validate Logical Table SID value. Verify if it matches Overlay ECMP
...
/*!
 * \brief ECMP_OVERLAY logical table get next entry.
 *
 * Traverses ECMP_OVERLAY logical table starting from the specified ECMP_ID
                               ...
                               const bcmltd_generic_arg_t *arg)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lth_overlay_group_next: LT_SID=%d.\n"),
              arg->sid));
    ...
    BCMECMP_ALLOC(in_entry, sizeof(*in_entry), "bcmecmpLthOlayGrpNxtInEnt");
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmecmp_olay_lt_fields_parse(unit, in, arg->sid, in_entry));
                                 ...
                                 const bcmltd_generic_arg_t *arg)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lth_overlay_group_commit: LT_SID=%d.\n"),
              arg->sid));
    ...
    for (ecmp_id = BCMECMP_LT_MIN_ECMP_ID
        ...
        if (BCMECMP_GRP_ECMP_NHOP(unit, ecmp_id)
            ...
            if (BCMECMP_TBL_REF_CNT(BCMECMP_TBL_PTR(unit, group,
                ...
                sal_memcpy(BCMECMP_GRP_UECMP_BK_PTR(unit, ecmp_id),
                           BCMECMP_GRP_UECMP_PTR(unit, ecmp_id),
                           (sizeof(bcmecmp_id_t) * BCMECMP_GRP_MAX_PATHS
                                                            (unit, ecmp_id)));
            } else {
                /* Delete commit. */
                                ...
                                const bcmltd_generic_arg_t *arg)
{
    ...
    LOG_INFO(BSL_LOG_MODULE,
             (BSL_META_U(unit,
                         "bcmecmp_lth_overlay_group_abort: LT_SID=%d.\n"),
              arg->sid));
        ...
        if (BCMECMP_GRP_ECMP_NHOP(unit, ecmp_id)
            ...
            if (BCMECMP_TBL_BK_REF_CNT(BCMECMP_TBL_PTR(unit, group,
                ...
                && BCMECMP_TBL_REF_CNT(BCMECMP_TBL_PTR(unit, group,
                                                    ...
                                                    ecmp_id) == 0) {
                /* Revert delete. */
                BCMECMP_ALLOC(BCMECMP_GRP_UECMP_PTR(unit, ecmp_id),
                              BCMECMP_GRP_MAX_PATHS_BK(unit, ecmp_id) *
                              sizeof(*(BCMECMP_GRP_UECMP_PTR(unit,
                                                             ecmp_id))),
                              "bcmecmpOlayRhGrpAbortGrpUecmpArr");
                sal_memcpy(BCMECMP_GRP_UECMP_PTR(unit, ecmp_id),
                           BCMECMP_GRP_UECMP_BK_PTR(unit, ecmp_id),
                           (sizeof(bcmecmp_id_t) * BCMECMP_GRP_MAX_PATHS_BK
                                                            (unit, ecmp_id)));
                        ...
                        && BCMECMP_TBL_REF_CNT(BCMECMP_TBL_PTR(unit, group,
                                                    ...
                                                    ecmp_id)) {
                /* Revert insert. */
                SHR_NULL_CHECK(BCMECMP_GRP_UECMP_PTR(unit, ecmp_id),
                                                    SHR_E_INTERNAL);
                BCMECMP_FREE(BCMECMP_GRP_UECMP_PTR(unit, ecmp_id));

                SHR_NULL_CHECK(BCMECMP_GRP_UECMP_BK_PTR(unit, ecmp_id),
                                                       SHR_E_INTERNAL);
                BCMECMP_FREE(BCMECMP_GRP_UECMP_BK_PTR(unit, ecmp_id));
            } else {
                /* Revert update. */
            ...
            if (BCMECMP_TBL_BK_REF_CNT(BCMECMP_TBL_PTR(unit, group,
                ...
                BCMECMP_ALLOC(BCMECMP_GRP_NHOP_PTR(unit, ecmp_id),
                              ...
                              sizeof(*(BCMECMP_GRP_NHOP_PTR(unit,
                                                            ecmp_id))),
                              "bcmecmpOlayRhGrpAbortGrpNhopArr");
