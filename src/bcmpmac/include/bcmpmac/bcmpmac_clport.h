                 ...
                 uint32_t init)
{
    ...
    /* Reset the MIB. */
    ioerr += READ_CLPORT_MIB_RESETr(pa, port, &mib_reset);
    fval = CLPORT_MIB_RESETr_CLR_CNTf_GET(mib_reset);
    fval |= 1 << port;
    CLPORT_MIB_RESETr_CLR_CNTf_SET(mib_reset, fval);
    ioerr += WRITE_CLPORT_MIB_RESETr(pa, port, mib_reset);
    fval &= ~(1 << port);
    CLPORT_MIB_RESETr_CLR_CNTf_SET(mib_reset, fval);
    ioerr += WRITE_CLPORT_MIB_RESETr(pa, port, mib_reset);
