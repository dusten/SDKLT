                 ...
                 uint32_t init)
{
    ...
    /* Reset the MIB. */
    ioerr += READ_XLPORT_MIB_RESETr(pa, port, &mib_reset);
    fval = XLPORT_MIB_RESETr_CLR_CNTf_GET(mib_reset);
    fval |= 1 << port;
    XLPORT_MIB_RESETr_CLR_CNTf_SET(mib_reset, fval);
    ioerr += WRITE_XLPORT_MIB_RESETr(pa, port, mib_reset);
    fval &= ~(1 << port);
    XLPORT_MIB_RESETr_CLR_CNTf_SET(mib_reset, fval);
    ioerr += WRITE_XLPORT_MIB_RESETr(pa, port, mib_reset);
...
xlport_lpbk_set(bcmpmac_access_t *pa, bcmpmac_pport_t port, uint32_t en)
{
    ...
    XLMAC_RX_LSS_CTRLr_LOCAL_FAULT_DISABLEf_SET(rx_lss_ctrl, en ? 0 : 1);
    XLMAC_RX_LSS_CTRLr_REMOTE_FAULT_DISABLEf_SET(rx_lss_ctrl, en ? 0 : 1);
                      ...
                      bcmpmac_mac_t mac)
{
    ...
    sa_hi = (mac[0] << 8) | mac[1];
    sa_lo = (mac[2] << 24) | (mac[3] << 16) | (mac[4] << 8) | mac[5];
                      ...
                      bcmpmac_mac_t mac)
{
    ...
    mac[0] = (uint8_t)(sa_hi >> 8);
    mac[1] = (uint8_t)(sa_hi);
    mac[2] = (uint8_t)(sa_lo >> 24);
    mac[3] = (uint8_t)(sa_lo >> 16);
    mac[4] = (uint8_t)(sa_lo >> 8);
    mac[5] = (uint8_t)(sa_lo);
