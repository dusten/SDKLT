 ...
 * attribute) was set, return all the fields that were set. Return
 * NULL if the sid is not a config logical table, or the config
 * logical table was not set at all.
 *
...
/*!
 * \brief Return a given fields for a config logical table
 *
 * If a field in a config logical table (which has a "config: TRUE"
 * attribute) was set, return the field value. Return NULL if the sid
 * is not a config logical table, fid is invalid, or the config
 * logical table was not set at all.
 *
 * \param [in]  unit            Unit number.
 * \param [in]  sid             Logical table sid.
