...
${FIGDIR}/%.png: ${DOCDIR}/%.fig
	mkdir -p $(dir $@)
	if [ -n "$(DITAA)" ] && [ -x $(DITAA) ]; then \
		cp $< ${FIGDIR}/; \
		echo "Creating image $@"; \
		unix2dos ${FIGDIR}/$(notdir $<) 2>/dev/null; \
		$(DITAA) -o ${FIGDIR}/$(notdir $<) >/dev/null; \
	else \
		echo "Image generation requires java-based ditaa tool (ditaa.sourceforge.net)"; \
	fi

# The construct below overrides the Doxyfile OUTPUT_DIRECTORY
doc: $(FIGS)
	@echo Create documentation in $(DOC_DSTDIR)
	( cat Doxyfile ; echo "OUTPUT_DIRECTORY=$(DOC_DSTDIR)" ) | $(DOXYGEN) - > /dev/null
