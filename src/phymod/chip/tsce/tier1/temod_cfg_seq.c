...
static int _temod_wait_sc_stats_set(PHYMOD_ST* pc);
...
int temod_set_pll_mode(PHYMOD_ST* pc, int pmd_touched, temod_spd_intfc_type spd_intf, int pll_mode)           /* SET_PLL_MODE */
{
  ...
  if (pmd_touched == 0) {
    temod_get_mapped_speed(spd_intf, &speed);
...
int temod_spd_intf_get(PHYMOD_ST* pc, int *spd_intf) 
{
  ...
  SC_X4_RSLVD_SPDr_CLR(sc_final_resolved_speed);
  READ_SC_X4_RSLVD_SPDr(pc,&sc_final_resolved_speed);
  *spd_intf = temod_get_actual_speed(SC_X4_RSLVD_SPDr_SPEEDf_GET(sc_final_resolved_speed), spd_intf); 
...
int temod_set_spd_intf(PHYMOD_ST *pc, temod_spd_intfc_type spd_intf, int no_trig)
{
  ...
  PHYMOD_IF_ERR_RETURN (temod_get_mapped_speed(spd_intf, &speed_id));
...
int temod_plldiv_lkup_get(PHYMOD_ST* pc, temod_spd_intfc_type spd_intf, phymod_ref_clk_t ref_clk,  uint32_t *plldiv, uint16_t *speed_vec)
{
  ...
  temod_get_mapped_speed(spd_intf, &speed_id);
  if (ref_clk == phymodRefClk125Mhz) {
