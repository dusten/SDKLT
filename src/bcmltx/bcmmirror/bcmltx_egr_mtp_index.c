...
{
    int   mirror_id, member;

    SHR_FUNC_ENTER(unit);

    LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META(
                "bcmltx_egr_mtp_index_rev_transform \n")));

    if ((in->count < 1) || (arg->rfields < 2)) {
        SHR_RETURN_VAL_EXIT(SHR_E_INTERNAL);
    }

    mirror_id = in->field[0]->data>>3;
    member    = in->field[0]->data&0x7;
                                ...
                                const bcmltd_transform_arg_t *arg)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META(
                "bcmltx_egr_mtp_index_transform \n")));
    ...
    out->field[0]->data = (mirror_id*8)+member;
