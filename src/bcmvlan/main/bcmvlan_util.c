...
int
bcmvlan_comp_config(int unit)
{
    SHR_FUNC_ENTER(unit);
    SHR_IF_ERR_EXIT(bcmvlan_xlate_port_grp_config(unit));
exit:
    SHR_FUNC_EXIT();
}
...
int
bcmvlan_ireq_read(int unit, bcmltd_sid_t lt_id,
...
int
bcmvlan_ireq_write(int unit, bcmltd_sid_t lt_id,
                   bcmdrd_sid_t pt_id, int index, void *entry_data)
{
    bcmbd_pt_dyn_info_t pt_info;
    bcmltd_sid_t rsp_ltid = 0;
