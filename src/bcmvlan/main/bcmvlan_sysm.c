...
bcmvlan_unit_init(shr_sysm_categories_t instance_category,
                   int unit,
                   void *comp_data,
                   bool warm,
...
/* Phase: shutdown */
static shr_sysm_rv_t
bcmvlan_unit_shutdown(shr_sysm_categories_t instance_category,
                       int unit,
                       void *comp_data,
                       bool warm,
                       uint32_t *blocking_component)
{
    ...
    rv = bcmlrd_map_get(unit, VLAN_XLATE_CONFIGt, &map);
    if (SHR_SUCCESS(rv) && map) {
        if (SHR_FAILURE(bcmvlan_drv_cleanup(unit))) {
...
/* Phase: comp_config */
static shr_sysm_rv_t
bcmvlan_unit_comp_config(shr_sysm_categories_t instance_category,
                         int unit,
                         void *comp_data,
                         bool warm,
                         uint32_t *blocking_component)
{
    ...
    rv = bcmlrd_map_get(unit, VLAN_XLATE_CONFIGt, &map);
    if (SHR_SUCCESS(rv) && map) {
        if (SHR_FAILURE(bcmvlan_drv_init(unit))) {
            return SHR_SYSM_RV_ERROR;
        }

        if (SHR_FAILURE(bcmvlan_comp_config(unit))) {
            return SHR_SYSM_RV_ERROR;
        }
    }
...
bcmvlan_start(void)
{
    ...
    funcs.init = bcmvlan_unit_init;
    funcs.shutdown = bcmvlan_unit_shutdown;
    funcs.comp_config = bcmvlan_unit_comp_config;
    SHR_IF_ERR_EXIT(
