...
/*!
 * \brief IFP meter validation
 *
 * Validate IFP Meter config parameters.
 *
                          ...
                          const bcmltd_generic_arg_t *arg)
{
    ...
    fid = METER_FP_ING_TEMPLATEt_METER_FP_ING_TEMPLATE_IDf;
    SHR_IF_ERR_EXIT
    ...
    if (meter_id <= 0 || meter_id > BCMMETER_FP_ING_METERS(unit)) {
...
/*!
 * \brief IFP meter insert
 *
 * Insert an IFP Meter in the harwdare meter table.
 *
 * \param [in]  unit          Unit Number.
                        ...
                        const bcmltd_generic_arg_t *arg)
{
     ...
     */
    if (BCMMETER_FP_ING_INDEX_USED_GET(unit, meter_id)) {
        SHR_RETURN_VAL_EXIT(SHR_E_EXISTS);
    }

    pipe_id = meter_entry.fields.pipe;
    SHR_IF_ERR_EXIT
        (bcmmeter_fp_oper_mode_get (unit, BCMMETER_FP_STAGE_ID_INGRESS, &oper_mode));
    ...
    if ((oper_mode == BCMMETER_FP_OPER_MODE_GLOBAL && pipe_id != 0) ||
        (oper_mode == BCMMETER_FP_OPER_MODE_PIPE_UNIQUE &&
         pipe_id >= BCMMETER_FP_ING_NUM_PIPES(unit))) {
        SHR_RETURN_VAL_EXIT(SHR_E_PARAM);
    }
     ...
     */
    ptr = BCMMETER_FP_ING_ENTRY_LOOKUP(unit, meter_id);
    if (ptr == NULL) {
     ...
     */
    BCMMETER_FP_ING_INDEX_USED_SET(unit, meter_id);
                        ...
                        const bcmltd_generic_arg_t *arg)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmmeter_entry_lookup(unit, BCMMETER_FP_STAGE_ID_INGRESS,
                               meter_id, &ptr));
                        ...
                        const bcmltd_generic_arg_t *arg)
{
     ...
     */
    BCMMETER_FP_ING_INDEX_USED_CLR(unit, meter_id);
    sal_memset(ptr, 0, sizeof(bcmmeter_fp_entry_t));
...
/*!
 * \brief IFP meter update
 *
 * Update an already configured IFP meter.
 *
 * \param [in]  unit          Unit Number.
 * \param [in]  trans_id      LT transaction ID.
 * \param [in]  in            Input field values.
                        ...
                        const bcmltd_generic_arg_t *arg)
{
    ...
    meter_entry = BCMMETER_FP_ING_ENTRY_LOOKUP(unit, meter_id);
    if (meter_entry == NULL) {
        SHR_RETURN_VAL_EXIT(SHR_E_FAIL);
    }
...
bcmmeter_fp_ing_attach (int unit)
{
    ...
    SHR_IF_ERR_EXIT
        (dev_fp_ing_attach[dev_type].attach(unit));
...
bcmmeter_fp_ing_detach (int unit)
{
    ...
    SHR_IF_ERR_EXIT
        (dev_fp_ing_detach[dev_type].detach(unit));
                     ...
                     bool cold)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmmeter_fp_ing_drv_get(unit, &drv));
...
bcmmeter_fp_ing_dev_config (int unit)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmcfg_field_get(unit,
                          METER_FP_ING_CONFIGt,
                          METER_FP_ING_CONFIGt_REFRESHf,
                          &refresh_en));
    SHR_IF_ERR_EXIT
                      ...
                      const bcmltd_generic_arg_t *arg)
{
    ...
    SHR_IF_ERR_EXIT
        (bcmlrd_table_field_def_get(
                unit,
                METER_FP_ING_TEMPLATEt,
                METER_FP_ING_TEMPLATEt_METER_FP_ING_TEMPLATE_IDf,
                &field_def));
    min = BCMMETER_FP_FIELD_MIN(field_def.dtag);

    for (idx = min; idx <= BCMMETER_FP_ING_METERS(unit); idx++) {
        if (BCMMETER_FP_ING_INDEX_USED_GET(unit, idx)) {
            meter_id = idx;
            break;
        }
    }

    if (idx > BCMMETER_FP_ING_METERS(unit)) {
        SHR_RETURN_VAL_EXIT(SHR_E_NOT_FOUND);
    }
                     ...
                     const bcmltd_generic_arg_t *arg)
{
    ...
    for (idx = (meter_id + 1); idx <= BCMMETER_FP_ING_METERS(unit); idx++) {
        if (BCMMETER_FP_ING_INDEX_USED_GET(unit, idx)) {
            meter_id = idx;
            break;
        }
                        ...
                        const bcmltd_generic_arg_t *arg)
{
    ...
    active_ptr = BCMMETER_FP_ING_ACTIVE_ENTRY(unit);
    bkp_ptr = BCMMETER_FP_ING_BKP_ENTRY(unit);
    hw_idx = BCMMETER_FP_ING_ACTIVE_HW_IDX(unit);
    bkp_hw_idx = BCMMETER_FP_ING_BKP_HW_IDX(unit);
    ...
    table_size = (BCMMETER_FP_ING_NUM_PIPES(unit) *
...
{
    bcmmeter_fp_ing_chip_attr[unit] = attr;
...
bcmmeter_fp_ing_drv_get (int unit,
                         bcmmeter_fp_chip_attributes_t **attr)
{
    *attr  = bcmmeter_fp_ing_chip_attr[unit];
