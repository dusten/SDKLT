/*! \file bcma_bcmbdcmd_cmicm_set.c
 *
 * CLI 'set' command for CMICm
 */
/*
 ...
 */
static int
iter_op(const bcmdrd_symbol_t *symbol, bcmdrd_sid_t enum_val, void *vptr)
{
    uint32_t and_masks[BCMDRD_MAX_PT_WSIZE];
    uint32_t or_masks[BCMDRD_MAX_PT_WSIZE];
    cmicm_iter_t *cmicm_iter = (cmicm_iter_t *)vptr;
    bcma_bcmbd_id_t sid;
    int ctoks_num = cmicm_iter->ctoks_num;
    bcma_cli_tokens_t *ctoks = cmicm_iter->ctoks;
    const char **fnames;
    int unit = cmicm_iter->unit;
    ...
    sal_memcpy(&sid, cmicm_iter->sid, sizeof(sid));
    ...
    bcma_bcmbd_cmicm_symop(unit, symbol, enum_val, &sid, and_masks, or_masks);

    return 0;
}

static int
cmicm_set(bcma_cli_t *cli, bcma_cli_args_t *args,
          bcma_cli_tokens_t *ctoks, int ctoks_num)
{
    ...
    sal_memset(&cmicm_iter, 0, sizeof(cmicm_iter));
    ...
    cmicm_iter.unit = unit;
    cmicm_iter.ctoks_num = ctoks_num;
    cmicm_iter.ctoks = &ctoks[data_start];
    cmicm_iter.sid = &sid;

    iter.name = sid.addr.name;
    iter.symbols = symbols;
    iter.function = iter_op;
    iter.vptr = &cmicm_iter;
...
int
bcma_bcmbdcmd_cmicm_set(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    rv = cmicm_set(cli, args, ctoks, ctoks_num);
