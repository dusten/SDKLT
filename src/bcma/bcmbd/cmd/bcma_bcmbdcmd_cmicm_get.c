/*! \file bcma_bcmbdcmd_cmicm_get.c
 *
 * CLI 'get' command for CMICm
 */
/*
...
iter_op(const bcmdrd_symbol_t *symbol, bcmdrd_sid_t enum_val, void *vptr)
{
    ...
    bcma_bcmbd_cmicm_symop(unit, symbol, enum_val, &sid, NULL, NULL);

    return 0;
}

static int
cmicm_get(bcma_cli_t *cli, bcma_cli_args_t *args,
          bcma_cli_tokens_t *ctoks, int ctoks_num)
{
    ...
    for (idx = 0; idx < ctoks_num && ctoks[idx].argc; idx++) {
        ...
        if (sal_strcmp("nz", ctoks[idx].argv[0]) == 0) {
            sid_flags |= BCMA_BCMBD_CMD_IDF_NONZERO;
        }
        else if (sal_strcmp("raw", ctoks[idx].argv[0]) == 0) {
            sid_flags |= BCMA_BCMBD_CMD_IDF_RAW;
        }
            ...
            if (bcma_bcmbd_parse_id(ctoks[idx].argv[0], &sid, 0) < 0) {
                return bcma_bcmbd_parse_error("identifier", args->argv[0]);
            }
        }
    }
    sid.flags = sid_flags;
...
int
bcma_bcmbdcmd_cmicm_get(bcma_cli_t *cli, bcma_cli_args_t *args)
{
    ...
    rv = cmicm_get(cli, args, ctoks, ctoks_num);
