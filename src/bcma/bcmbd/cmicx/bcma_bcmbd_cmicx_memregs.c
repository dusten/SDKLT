                ...
                const char *errstr, int rc)
{
    ...
    if (p >= 0) {
        if (sect_size > 0) {
            ...
            if (bcma_bcmbd_parse_is_int(sid->port.name)) {
                cli_out(".%d (BLOCK %d, PORT %s)", p, b, port_str);
            } else {
 ...
 */
static int
cmicx_cmd_regop(int unit, const bcmdrd_symbol_t *symbol,
                bcma_bcmbd_id_t *sid, int aindex, int block, int port,
                int pipe, uint32_t size,
 ...
 */
static int
cmicx_cmd_memop(int unit, const bcmdrd_symbol_t *symbol,
                bcma_bcmbd_id_t *sid,
                ...
                uint32_t size, uint32_t *and_masks, uint32_t *or_masks)
{
    ...
    if (rc < 0) {
        print_memreg_op(unit, "memory", sid, mindex, block, basetype,
                        pipe, sect_size, adext, addr + idx, "reading", rc);
        cli_out("\n");
        return rc;
    }
    ...
    if (and_masks || or_masks) {
        ...
        if (rc < 0) {
            print_memreg_op(unit, "memory", sid, mindex, block, basetype,
                            pipe, sect_size, adext, addr + idx, "writing", rc);
            cli_out("\n");
            return rc;
        }
        ...
        for (b = sid->block.start; b <= sid->block.end; b++) {
            ...
            for (t = sid->pipe.start; t <= sid->pipe.end; t++) {
                ...
                for (p = sid->port.start; p <= sid->port.end; p++) {
                    ...
                    cmicx_cmd_regop(unit, symbol, sid, a, b, p, t, size,
                                    and_masks, or_masks);
                }
            }
        }
        ...
        for (t = sid->pipe.start; t <= sid->pipe.end; t++) {
            ...
            for (p = sid->port.start; p <= sid->port.end; p++){
                ...
                for (idx = sid->addr.start; idx <= sid->addr.end; idx++) {
                    cmicx_cmd_memop(unit, symbol, sid, b, idx + pipe_offset,
                                    t, p, sect_size, size, and_masks, or_masks);
                }
            }
        }
    }
