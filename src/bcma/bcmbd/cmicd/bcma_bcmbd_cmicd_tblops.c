 ...
 */
static int
cmicd_cmd_tblop(int unit, const bcmdrd_symbol_t *symbol,
                ...
                cmicd_tbl_op_t tbl_op)
{
    ...
    if (and_masks || or_masks) {
        ...
        index = CMICD_RESP_WORD_INDEX_GET(resp_info.resp_word);
             ...
             cmicd_tbl_op_t tbl_op)
{
    ...
    for (b = sid->block.start; b <= sid->block.end; b++) {
        ...
        for (t = sid->pipe.start; t <= sid->pipe.end; t++) {
            ...
            cmicd_cmd_tblop(unit, symbol, sid, b, t, size, and_masks, or_masks,
                            tbl_op);
        }
    }
    return 0;
