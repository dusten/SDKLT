 ...
 */
static int
cmicd_cmd_regop(int unit, const bcmdrd_symbol_t *symbol,
 ...
 */
static int
cmicd_cmd_memop(int unit, const bcmdrd_symbol_t *symbol,
                bcma_bcmbd_id_t *sid,
        ...
        for (b = sid->block.start; b <= sid->block.end; b++) {
            ...
            for (t = sid->pipe.start; t <= sid->pipe.end; t++) {
                ...
                for (p = sid->port.start; p <= sid->port.end; p++) {
                    ...
                    cmicd_cmd_regop(unit, symbol, sid, a, b, p, t, size,
                                    and_masks, or_masks);
                }
            }
        }
        ...
        for (t = sid->pipe.start; t <= sid->pipe.end; t++) {
            ...
            for (p = sid->port.start; p <= sid->port.end; p++){
                ...
                for (idx = sid->addr.start; idx <= sid->addr.end; idx++) {
                    cmicd_cmd_memop(unit, symbol, sid, b, idx + pipe_offset,
