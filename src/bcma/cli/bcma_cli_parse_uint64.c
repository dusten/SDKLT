...
bcma_cli_parse_uint64(const char *str, uint64_t *val)
{
    ...
    if (val) {
        ...
        if (str[0] == '0' && (str[1] == 'b' || str[1] == 'B')) {
            *val = (uint64_t)sal_strtoull(&str[2], NULL, 2);
        }

        /* Parse octal as unsigned */
        if (str[0] == '0' && (str[1] >= '0' && str[1] <= '7')) {
            *val = (uint64_t)sal_strtoull(str, NULL, 8);
        }

        /* Parse hexadecimal as unsigned */
        if (str[0] == '0' && (str[1] == 'x' || str[1] == 'X')) {
            *val = (uint64_t)sal_strtoull(&str[2], NULL, 16);
        }

        /* Parse decimal as unsigned */
        *val = (uint64_t)sal_strtoull(str, NULL, 0);
    }
    return 0;
}
