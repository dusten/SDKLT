...
bcmltm_tree_node_create(void *node_data)
{
    ...
    new_node = sal_alloc(sizeof(bcmltm_node_t), "LTM tree node");
                             ...
                             void *node_data)
{
    ...
    parent_node->left = bcmltm_tree_node_create(node_data);
                              ...
                              void *node_data)
{
    ...
    parent_node->right = bcmltm_tree_node_create(node_data);

    return parent_node->right;
}

/*
 * Traverse the binary tree, executing the provided function on the node
 * data at each step.
 * This uses post-order recursive traversal of the tree.
 */
int
                     ...
                     void *traverse_cookie)
{
    ...
    if (node->left != NULL) {
        rv = bcmltm_tree_traverse(unit, node->left,
                                  traverse_op, traverse_cookie);
        if (SHR_E_NONE != rv) {
            return rv;
        }
    ...
    if (node->right != NULL) {
        rv = bcmltm_tree_traverse(unit, node->right,
                                  traverse_op, traverse_cookie);
        if (SHR_E_NONE != rv) {
            return rv;
        }
    ...
    if (node->node_data != NULL) {
        rv = (*traverse_op)(unit, node->node_data, traverse_cookie);
    }
...
{
    /* @@@ Free the node memory in some way @@@ */
    if (node != NULL) {
        sal_free(node);
    }
}
...
bcmltm_tree_destroy(bcmltm_node_t *node)
{
    ...
    if (node->left != NULL) {
        bcmltm_tree_destroy(node->left);
    }

    if (node->right != NULL) {
        bcmltm_tree_destroy(node->right);
    }
