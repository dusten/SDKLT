                              ...
                              bcmltm_fa_node_t **fa_node_ptr)
{
    ...
    /* Allocate */
    SHR_ALLOC(fa_node, sizeof(*fa_node), "LTM Logical Table fa_node");
 ...
 exit:
    if (SHR_FUNC_ERR()) {
        bcmltm_md_fa_node_data_destroy(fa_node);
        *fa_node_ptr = NULL;
    }
...
bcmltm_md_fa_tree_node_data_destroy(bcmltm_node_t *node)
{
    ...
    if (node->left != NULL) {
        bcmltm_md_fa_tree_node_data_destroy(node->left);
    }

    if (node->right != NULL) {
        bcmltm_md_fa_tree_node_data_destroy(node->right);
    }

    if (node->node_data != NULL) {
        bcmltm_md_fa_node_data_destroy(node->node_data);
        node->node_data = NULL;
    }
                                ...
                                bcmltm_node_t **fa_node_roots)
{
    ...
    for (root = 0; root < num_roots; root++) {
         ...
         */
        bcmltm_md_fa_tree_node_data_destroy(fa_node_roots[root]);

        /* Destroy tree */
        bcmltm_tree_destroy(fa_node_roots[root]);
    }

    SHR_FREE(fa_node_roots);
                              ...
                              bcmltm_ee_node_t **ee_node_ptr)
{
    ...
    /* Allocate */
    SHR_ALLOC(ee_node, sizeof(*ee_node), "LTM Logical Table ee_node");
 ...
 exit:
    if (SHR_FUNC_ERR()) {
        bcmltm_md_ee_node_data_destroy(ee_node);
        *ee_node_ptr = NULL;
    }
...
bcmltm_md_ee_tree_node_data_destroy(bcmltm_node_t *node)
{
    ...
    if (node->left != NULL) {
        bcmltm_md_ee_tree_node_data_destroy(node->left);
    }

    if (node->right != NULL) {
        bcmltm_md_ee_tree_node_data_destroy(node->right);
    }

    if (node->node_data != NULL) {
        bcmltm_md_ee_node_data_destroy(node->node_data);
        node->node_data = NULL;
    }
                                ...
                                bcmltm_node_t **ee_node_roots)
{
    ...
    for (root = 0; root < num_roots; root++) {
         ...
         * the node_data so this needs to be explicitly done.
         */
        bcmltm_md_ee_tree_node_data_destroy(ee_node_roots[root]);

        /* Destroy tree */
        bcmltm_tree_destroy(ee_node_roots[root]);
    }

    SHR_FREE(ee_node_roots);
...
bcmltm_md_op_destroy(bcmltm_lt_op_md_t *op_md)
{
    ...
    bcmltm_md_ee_node_roots_destroy(op_md->num_roots, op_md->ee_node_roots);
    bcmltm_md_fa_node_roots_destroy(op_md->num_roots, op_md->fa_node_roots);
