/*! \file bcmltm_nc_lta_xfrm.h
 *
 * Logical Table Manager - Node Cookies.
 *
 * Logical Table mappings with Field Transform groups definitions.
 *
 * This file contains routines to construct LT information
 * required by the LTM metadata for LT with LTA Field Transform Handler.
 */
/*
...
#ifndef BCMLTM_NC_LTA_XFRM_H
#define BCMLTM_NC_LTA_XFRM_H
...
typedef struct bcmltm_nc_lta_xfrm_info_s {
    ...
    /*!
     * LTA in field list used as the LTA FA node cookie.
     * (LTM to/from LTA).
     *
     * This information can be shared among several BCMLTM_MD operations
     * for a given symbol.
    ...
    /*!
     * LTA out field list used as the LTA FA node cookie.
     * (LTA to/from PTM).
     *
     * This information can be shared among several BCMLTM_MD operations
     * for a given symbol.
 ...
 * nodes.
 */
typedef struct bcmltm_nc_lta_xfrm_info_list_s {
    /*!
     * Number of transform pair.
     */
    uint32_t num;

    /*!
     * Array of Field Transform node cookie information.
     */
    bcmltm_nc_lta_xfrm_info_t *xfrm_info;

} bcmltm_nc_lta_xfrm_info_list_t;

/*!
 * \brief Create LT cookies info for Field Transform Handler for given table.
 *
 * Create the LT cookies information needed by the LTM metadata
 * for LTA Field Transform Handler.
 *
 * \param [in] unit Unit number.
 * \param [in] sid Table ID.
 * \param [in] attr Table attributes.
 * \param [in] xfrm_type Field Transform Type : Forward or Reverse Transform.
 * \param [in] entries map information.
 * \param [in] pt_map_group PT map group information.
 * \param [out] info_ptr Returning pointer to LT info.
 *
...
bcmltm_nc_lta_xfrm_info_create(int unit, bcmlrd_sid_t sid,
                               ...
                               bcmltm_nc_lta_xfrm_info_list_t **info_ptr);

/*!
 * \brief Create LTA-LTA copy LT cookies info for Field Transform Handler
 *        for given table.
 *
 ...
 * \param [out] info_ptr Returning pointer to LT info for xform copy.
 *
 * \retval SHR_E_NONE No errors.
 * \retval !SHR_E_NONE Failure.
 */
...
/*!
 * \brief Destroy LT cookies info for Field Transform map groups.
 *
 * Destroy LT cookies info for LTA Field Transform.
 *
 * \param [in] info Pointer to LT info map groups to destroy.
 */
extern void
bcmltm_nc_lta_xfrm_info_destroy(bcmltm_nc_lta_xfrm_info_list_t *info_ptr);


#endif /* BCMLTM_NC_LTA_XFRM_H */
