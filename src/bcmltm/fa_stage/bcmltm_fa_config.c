                             ...
                             void *node_cookie)
{
    ...
    if (lt_entry->flags & BCMLTM_ENTRY_FLAG_TRAVERSE_START) {
        ...
        /* Previous LTID was copied to WB */
        target_ltid = ltm_buffer[idx_info->lt_index_offset] + 1;
    }
    ...
    do {
        ...
        if (rv != SHR_E_NOT_FOUND) {
            /* This particular LTID is not on this unit. */
            break;
        }
        target_ltid++;
    } while (1);
    ...
    if (SHR_SUCCESS(rv)) {
        /* Copy LTID to index location in WB */
        ltm_buffer[idx_info->lt_index_offset] = target_ltid;
