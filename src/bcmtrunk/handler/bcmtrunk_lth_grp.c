...
#include <bcmtrunk/bcmtrunk_lth_grp.h>
...
static bcmtrunk_lth_grp_bk_t *lth_grp_bk[BCMDRD_CONFIG_MAX_UNITS];

/*! TRUNK bookkeeping temporary structure. */
static bcmtrunk_lth_grp_bk_t * lth_grp_bk_temp[BCMDRD_CONFIG_MAX_UNITS];

/*! TRUNK group SW bitmap changed in one commit. */
SHR_BITDCL *bcmtrunk_grp_bitmap_diff[BCMDRD_CONFIG_MAX_UNITS];
...
/*! Pointer to trunk group temporary state. */
#define GRP_DIFF(unit)               bcmtrunk_grp_bitmap_diff[unit]
 ...
 */
int
bcmtrunk_field_parse(int unit, const bcmltd_fields_t *in,
                     bcmtrunk_group_param_t **param)
{
    ...
    (*param) = sal_alloc(sizeof(bcmtrunk_group_param_t),
                         "bcmtrunk_group_param_t");
    if ((*param) == NULL) {
        SHR_RETURN_VAL_EXIT(SHR_E_MEMORY);
    }
    sal_memset((*param), 0, sizeof(bcmtrunk_group_param_t));


    LOG_VERBOSE(BSL_LOG_MODULE,
        (BSL_META_U(unit, "bcmtrunk_field_parse:CNT=%d.\n"),
                    (uint32_t)(in->count)));
    ...
    for (idx = 0; idx < in->count; idx++) {
        ...
        switch (id) {
            ...
            case TRUNKt_UC_MAX_MEMBERSf:
                if ((fval < BCMTRUNK_MIN_MEMBERS) ||
                    (fval > BCMTRUNK_MAX_MEMBERS)) {
                    SHR_RETURN_VAL_EXIT(SHR_E_PARAM);
                }
            ...
            case TRUNKt_UC_MEMBER_MODIDf:
                if ((arr_idx) >= BCMTRUNK_MAX_MEMBERS) {
                    LOG_VERBOSE(BSL_LOG_MODULE,
            ...
            case TRUNKt_UC_MEMBER_MODPORTf:
                ...
                LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "TRUNKt_UC_MODPORTf[%d] = %d.\n"),
                                arr_idx, fval));
                break;
            case TRUNKt_UC_AGM_IDf:
                (*param)->uc_agm_id = fval;
                (*param)->uc_agm_id_valid = TRUE;
                LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U(unit, "TRUNKt_UC_AGM_IDf = %d.\n"), fval));
                break;
             case TRUNKt_NONUC_MEMBER_CNTf:
                if (fval > BCMTRUNK_MAX_MEMBERS) {
                    SHR_RETURN_VAL_EXIT(SHR_E_PARAM);
                }
...
int
bcmtrunk_grp_to_api(int unit, bcmtrunk_group_t *grp, bcmltd_fields_t *out)
{
    uint32_t idx = 0, px;
    ...
    for (px = 0; px < BCMTRUNK_MAX_MEMBERS; px++) {
    ...
    out->field[idx]->id   = TRUNKt_UC_AGM_IDf;
...
static int
bcmtrunk_sw_update(int unit, bcmtrunk_group_param_t *param,
...
int
bcmtrunk_lth_grp_validate(int unit,
                          bcmlt_opcode_t opcode,
                          const bcmltd_fields_t *in,
                          const bcmltd_generic_arg_t *arg)
{
    bcmtrunk_group_param_t *param = NULL;
    ...
    SHR_IF_ERR_EXIT(
        bcmtrunk_field_parse(unit, in, &param));
    if (!(param->id_valid)) {
       SHR_RETURN_VAL_EXIT(SHR_E_PARAM);
    }
    trunk_id = param->id;
    ...
    switch (opcode) {
        case BCMLT_OPCODE_INSERT:
            ...
            if (!param->uc_max_members_valid) {
                param->uc_max_members_valid = TRUE;
                param->uc_max_members = BCMTRUNK_MAX_MEMBERS;
            }
...
int
bcmtrunk_lth_grp_insert(int unit,
                        uint32_t trans_id,
                        const bcmltd_fields_t *in,
                        const bcmltd_generic_arg_t *arg)
{
    ...
    SHR_IF_ERR_EXIT(
        bcmtrunk_grp_add(unit, trans_id, TRUNKt, GRP_TEMP(unit), param));

    SHR_IF_ERR_EXIT(
        bcmtrunk_sw_update(unit, param, BCMTRUNK_GRP(unit, trunk_id)));
...
int
bcmtrunk_lth_grp_lookup(int unit,
                        uint32_t trans_id,
                        const bcmltd_fields_t *in,
                        bcmltd_fields_t *out,
                        const bcmltd_generic_arg_t *arg)
{
    ...
    SHR_IF_ERR_EXIT(
        bcmtrunk_grp_to_api(unit, BCMTRUNK_GRP(unit, trunk_id), out));
...
int
bcmtrunk_lth_grp_delete(int unit,
                        uint32_t trans_id,
                        const bcmltd_fields_t *in,
                        const bcmltd_generic_arg_t *arg)
{
    ...
    SHR_IF_ERR_EXIT(bcmtrunk_field_parse(unit, in, &param));
    if (!(param->id_valid)) {
        SHR_RETURN_VAL_EXIT(SHR_E_PARAM);
    }
    trunk_id = param->id;
    SHR_IF_ERR_VERBOSE_EXIT(
        bcmtrunk_grp_del(unit, trans_id, TRUNKt, GRP_TEMP(unit), param));

    sal_memset(BCMTRUNK_GRP(unit, trunk_id), 0, sizeof(bcmtrunk_group_t));
...
int
bcmtrunk_lth_grp_update(int unit,
                        uint32_t trans_id,
                        const bcmltd_fields_t *in,
                        const bcmltd_generic_arg_t *arg)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT(
        bcmtrunk_grp_update(unit, trans_id, TRUNKt, GRP_TEMP(unit), param));
...
int
bcmtrunk_lth_grp_first(int unit,
                       uint32_t trans_id,
                       bcmltd_fields_t *out,
                       const bcmltd_generic_arg_t *arg)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmtrunk_lth_field_value_range_get(unit, TRUNKt, TRUNKt_TRUNK_IDf,
                                            &min, &max));
    ...
    for (trunk_id = min; trunk_id < max; trunk_id++) {
        if (GRP_EXIST(unit, trunk_id)) {
            ...
            flds.count = 1;
            SHR_IF_ERR_VERBOSE_EXIT
                (bcmtrunk_lth_grp_lookup(unit, trans_id, &flds, out, arg));
...
int
bcmtrunk_lth_grp_next(int unit,
                      uint32_t trans_id,
                      const bcmltd_fields_t *in,
                      bcmltd_fields_t *out,
                      const bcmltd_generic_arg_t *arg)
{
    ...
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmtrunk_lth_field_value_range_get(unit, TRUNKt, TRUNKt_TRUNK_IDf,
                                            &min, &max));
    SHR_IF_ERR_VERBOSE_EXIT
        (bcmtrunk_lth_field_get(unit, in, TRUNKt_TRUNK_IDf, &trunk_id));
...
int
bcmtrunk_lth_grp_commit(int unit,
                        uint32_t trans_id,
                        const bcmltd_generic_arg_t *arg)
{
    uint32_t idx;
...
int
bcmtrunk_lth_grp_abort(int unit,
                       uint32_t trans_id,
                       const bcmltd_generic_arg_t *arg)
{
    uint32_t idx;
...
int
bcmtrunk_lth_grp_init(int unit, bool warm)
{
    ...
    if (!GRP_HA(unit)) {
        alloc_size = sizeof(bcmtrunk_lth_grp_bk_t);
        SHR_ALLOC(GRP_HA(unit), alloc_size, "bcmtrunk_lth_grp_bk_t");
        SHR_NULL_CHECK(GRP_HA(unit), SHR_E_MEMORY);
        sal_memset(GRP_HA(unit), 0, alloc_size);

        SHR_ALLOC(GRP_TEMP(unit), alloc_size, "bcmtrunk_lth_grp_bk_t temp");
        SHR_NULL_CHECK(GRP_TEMP(unit), SHR_E_MEMORY);
        sal_memset(GRP_TEMP(unit), 0, alloc_size);

        SHR_IF_ERR_VERBOSE_EXIT(bcmtrunk_grp_init(unit, GRP_TEMP(unit)));
        GRP_HA(unit)->member_size = GRP_TEMP(unit)->member_size;

        /* HA structure. */
        ha_req_size = BCMTRUNK_MAX_TRUNK * sizeof(bcmtrunk_group_t);
        ha_alloc_size = ha_req_size;
        GRP_HA(unit)->grp = shr_ha_mem_alloc(unit,
                                             BCMMGMT_TRUNK_COMP_ID,
                                             BCMTRUNK_GRP_SUB_COMP_ID,
                                             &ha_alloc_size);
        ...
        GRP_HA(unit)->mbmp = shr_ha_mem_alloc(unit,
                                             BCMMGMT_TRUNK_COMP_ID,
                                             BCMTRUNK_MEMBER_BMP_SUB_COMP_ID,
                                             &ha_alloc_size);
        ...
        /* Temporary structure. */
        alloc_size = BCMTRUNK_MAX_TRUNK * sizeof(bcmtrunk_group_t);
        SHR_ALLOC(GRP_TEMP(unit)->grp, alloc_size, "bcmtrunk temp group info");
        ...
        SHR_ALLOC(GRP_TEMP(unit)->mbmp, alloc_size, "bcmtrunk member bmp info");
        ...
        SHR_ALLOC(GRP_DIFF(unit), alloc_size, "trunk bitmap diff");
...
exit:
    if (SHR_FUNC_ERR()) {
        (void) bcmtrunk_lth_grp_cleanup(unit);
    }
    SHR_FUNC_EXIT();
}

int
bcmtrunk_lth_grp_cleanup(int unit)
{
    SHR_FUNC_ENTER(unit);
