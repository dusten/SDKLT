...
bcmlrd_unit_init(void)
{
    ...
    for (unit = 0; unit < BCMDRD_CONFIG_MAX_UNITS; unit++) {
        ...
        if (dev_type == BCMDRD_DEV_T_NONE) {
            /* No device, so no configuration. */
            continue;
        }

        if (bcmlrd_dev_conf[dev_type].conf == NULL) {
            /* No configuration for this device. */
            continue;
        }

        if (bcmlrd_dev_conf[dev_type].conf->probe != NULL) {
            probe = bcmlrd_dev_conf[dev_type].conf->probe(unit);
        } else {
            probe = 0;
        }
        ...
        /* Configuration found */
        rv = bcmlrd_unit_conf_prepare(unit,
                                      bcmlrd_dev_conf[dev_type].conf,
                                      bcmlrd_unit_conf + unit);
