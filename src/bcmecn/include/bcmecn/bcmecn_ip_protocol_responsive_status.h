/*! \file bcmecn_ip_protocol_responsive_status.h
 *
 * ECN IP protocol responsive status Table Handler
 * This file contains ecn ip protocol responsive status table handler
 */
/*
...
#ifndef BCMECN_IP_PROTOCOL_RESPONSIVE_STATUS_H
#define BCMECN_IP_PROTOCOL_RESPONSIVE_STATUS_H

#include <bcmltd/bcmltd_handler.h>

/*! \cond  Externs for the required functions. */
#define BCMDRD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_cn,_pf,_pd,_r0,_r1) \
extern int _bc##_ecn_drv_attach(int unit);
...
/*! \cond  Externs for the required functions. */
#define BCMDRD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_cn,_pf,_pd,_r0,_r1) \
extern int _bc##_ecn_drv_detach(int unit);
#define BCMDRD_DEVLIST_OVERRIDE
#include <bcmdrd/bcmdrd_devlist.h>
/*! \endcond */

/*!
 * \brief ecn ip protocol responsive status table validation
 *
...
/*!
 * \brief ecn ip protocol responsive status table insert
 *
 * The method inserts user configuration for setting
...
/*!
 * \brief ecn ip protocol responsive status table lookup
 *
 * The method looks the incoming data and finds
 * matching entry.
 * User sends ip protocol and this method
 * gets responsive status of that protocol and
...
/*!
 * \brief ecn ip protocol responsive status table delete
 *
 * The method looks the incoming data and finds
...
/*!
 * \brief ecn ip protocol responsive status table update
 *
...
/*!
 * \brief ecn ip protocol responsive status table get first
 *
 * The method finds the first entry in the LT,
 * gets responsive status of that protocol and
 * puts into outgoing buffer.
 *
 * \param [in]  unit          Unit Number.
...
/*!
 * \brief ecn ip protocol responsive status table get next
 *
 * The method looks at the incoming data and finds
 * the next entry.
 * User sends ip protocol and this method
 * gets responsive status of the next protocol and
 * puts into outgoing buffer.
 *
 * \param [in]  unit          Unit Number.
 * \param [in]  trans_id      LT transaction ID.
 ...
 *
 * This method discards the backup state and copies the active
 * state to backup.
 *
 * \param [in] unit      Unit number.
...
#endif /* BCMECN_IP_PROTOCOL_RESPONSIVE_STATUS_H */
