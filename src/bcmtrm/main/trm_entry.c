 ...
 */
static void proc_lt_entry_results(bcmtrm_entry_t *entry,
                                  bcmltm_entry_t *ltm_entry)
{
    ...
    while (out_field) {
        ...
        if (entry->fld_arr[id]) {
                ...
                if (in_field->idx < idx) {
                    ...
                    while (true) {
                        ...
                        if (!in_field->next ||
                            ...
                            prev_out_field = out_field;
                            out_field = out_field->next;
                            prev_out_field->next = in_field->next;
                            in_field->next = prev_out_field;
                            break;
                        }
                        /* Case (2) above */
                    ...
                    if (entry->l_field == in_field) {
                        ...
                        prev_in_field->next = out_field;
                        out_field = out_field->next;
                        prev_in_field->next->next = in_field;
                    }
                    /*
                     * Update the input field to start at the begining of the
                     * array
                     */
                    in_field = entry->fld_arr[id];
                }
            ...
            entry->fld_arr[id]->next = entry->l_field;
            entry->l_field = entry->fld_arr[id];
            is_array = false;
        }
    }
}
                               ...
                               bcmltm_entry_t *ltm_entry)
{
    ...
    if (ltm_entry && (ltm_entry->out_fields != NULL)) {
        if (validate_input_field_order(ltm_entry->out_fields) == SHR_E_NONE) {
            ...
            if (entry->fld_arr) {
                proc_lt_entry_results(entry, ltm_entry);
            } else {
                        ...
                        uint32_t trans_id)
{
    ...
    if (entry->pt) {
        ltm_entry->flags = BCMLTM_ENTRY_FLAG_PASSTHRU;
    } else {
        ltm_entry->flags = 0;
    }
    if (entry->attrib & TRM_E_ATTR_HW_GET) {
        ltm_entry->flags |= BCMLTM_ENTRY_FLAG_HW_GET;
    }
    if (entry->attrib & TRM_E_ATTR_EXC_DEF) {
        ltm_entry->flags |= BCMLTM_ENTRY_FLAG_EXC_DEF;
    }
