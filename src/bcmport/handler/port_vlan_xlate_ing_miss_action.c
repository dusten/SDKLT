/*! \file port_vlan_xlate_ing_miss_action.c
 *
...
#include <bcmport/bcmport_port_vlan_xlate_ing_miss_action.h>
 ...
 */
int
bcmport_port_vlan_xlate_ing_miss_action_transform(int unit,
                                                  ...
                                                  const bcmltd_transform_arg_t *arg)
{
    ...
    uint32_t  vt_miss_drop_key_1_id = VT_MISS_DROP_KEY_1f;
    uint32_t  vt_miss_drop_key_2_id = VT_MISS_DROP_KEY_2f;
    uint32_t  vt_miss_drop_id = VT_MISS_DROPf;
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            "\t bcmport_port_vlan_xlate_ing_miss_action_transform\n")));
    if (in->count == 0) {
        goto exit;
    }
    ...
    out->count = 0;
    out->field[out->count]->id = vt_miss_drop_id;
    out->field[out->count]->data = vt_miss_drop_val;
    out->count++;
    out->field[out->count]->id = vt_miss_drop_key_1_id;
    out->field[out->count]->data = vt_miss_drop_key_1_val;
    out->count++;
    out->field[out->count]->id = vt_miss_drop_key_2_id;
 ...
 */
int
bcmport_port_vlan_xlate_ing_miss_action_rev_transform(int unit,
                                                      const bcmltd_fields_t *in,
                                                      bcmltd_fields_t *out,
                                                      const bcmltd_transform_arg_t *arg)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE,
                (BSL_META_U(unit,
                            "\t bcmport_port_vlan_xlate_ing_miss_action_rev_transform\n")));
    if (in->count == 0) {
        goto exit;
    }

    for (idx = 0; idx < in->count; idx++) {
        if (in->field[idx]->id == vt_miss_drop_id) {
            vt_miss_drop_val = in->field[idx]->data;
        } else if (in->field[idx]->id == vt_miss_drop_key_1_id) {
            vt_miss_drop_key_1_val = in->field[idx]->data;
        }  else if (in->field[idx]->id == vt_miss_drop_key_2_id) {
