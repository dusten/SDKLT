...
#define BSL_LOG_MODULE BSL_LS_BCMPORT_SYSM
                 ...
                 uint32_t *blocking_component)
{
    ...
    if (SHR_FAILURE(bcmport_mirror_init(unit, warm))) {
        return SHR_SYSM_RV_ERROR;
    }

    if (SHR_FAILURE(bcmport_imm_db_init(unit))) {
        return SHR_SYSM_RV_ERROR;
    }

    if (SHR_FAILURE(bcmport_imm_register(unit))) {
        return SHR_SYSM_RV_ERROR;
    }
