 ...
 */
void
bcmcnet_hal_free(struct pdma_dev *dev, uint32_t size, void *mem, dma_addr_t dma)
{
...
{
    struct pdma_dev *dev = rxq->ctrl->dev;
...
{
    struct pdma_dev *dev = txq->ctrl->dev;
    struct pkt_buf *pkb = (struct pkt_buf *)((bcmpkt_data_buf_t *)buf)->data;

    pbuf->len = pkb->pkh.data_len + (pbuf->adj ? pkb->pkh.meta_len : 0);
    pbuf->dma = (dma_addr_t)bcmdrd_hal_dma_map(dev->unit, &pkb->data + (pbuf->adj ? 0 : pkb->pkh.meta_len),
                                               pbuf->len, TRUE);
    if (!pbuf->dma) {
