...
#define BCMDRD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_cn,_pf,_pd,_r0,_r1) \
    { _bc##_tnl_encap_drv_attach },
static struct {
    bcmtnl_encap_drv_attach_f attach;
} dev_tnl_encap_attach[] = {
...
#define BCMDRD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_cn,_pf,_pd,_r0,_r1) \
    { _bc##_tnl_encap_drv_detach },
static struct {
    bcmtnl_encap_drv_detach_f detach;
} dev_tnl_encap_detach[] = {
    { 0 }, /* dummy entry for type "none" */
#include <bcmdrd/bcmdrd_devlist.h>
    { 0 } /* end-of-list */
};
...
#define BCMDRD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_cn,_pf,_pd,_r0,_r1) \
    { _bc##_tnl_decap_drv_attach },
static struct {
    bcmtnl_decap_drv_attach_f attach;
} dev_tnl_decap_attach[] = {
    { 0 }, /* dummy entry for type "none" */
#include <bcmdrd/bcmdrd_devlist.h>
    { 0 } /* end-of-list */
};
...
#define BCMDRD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_cn,_pf,_pd,_r0,_r1) \
    { _bc##_tnl_decap_drv_detach },
static struct {
    bcmtnl_decap_drv_detach_f detach;
} dev_tnl_decap_detach[] = {
    { 0 }, /* dummy entry for type "none" */
#include <bcmdrd/bcmdrd_devlist.h>
    { 0 } /* end-of-list */
};
...
bcmtnl_drv_attach (int unit)
{
    ...
    SHR_IF_ERR_EXIT
        (dev_tnl_encap_attach[dev_type].attach(unit));

    SHR_IF_ERR_EXIT
        (dev_tnl_decap_attach[dev_type].attach(unit));
...
bcmtnl_drv_detach (int unit)
{
    ...
    SHR_IF_ERR_EXIT
        (dev_tnl_encap_detach[dev_type].detach(unit));

    SHR_IF_ERR_EXIT
        (dev_tnl_decap_detach[dev_type].detach(unit));
  exit:
    SHR_FUNC_EXIT();
}
 ...
 */
static int
bcmtnl_init(int unit, bool warm)
{
    int                 rv = SHR_E_NONE;
    const bcmlrd_map_t  *map = NULL;
    bcmtnl_encap_drv_t  *drv = NULL;

    SHR_FUNC_ENTER(unit);

    if (bcmtnl_drv_attach(unit) != SHR_E_NONE) {
        return SHR_SYSM_RV_ERROR;
    }

    rv = bcmtnl_encap_drv_get(unit, &drv);
    if (SHR_SUCCESS(rv) && (drv != NULL)) {
        SHR_IF_ERR_EXIT(bcmtnl_sw_state_init(unit, warm));
    }

    rv = bcmlrd_map_get(unit, L3_EIFt, &map);
    if (SHR_SUCCESS(rv) && map) {
        SHR_IF_ERR_EXIT(bcmtnl_l3_intf_sw_state_init(unit, warm));
    }
...
bcmtnl_cleanup(int unit)
{
    ...
    rv = bcmtnl_encap_drv_get(unit, &drv);
    if (SHR_SUCCESS(rv) && (drv != NULL)) {
        SHR_IF_ERR_EXIT(bcmtnl_sw_state_cleanup(unit));
    }

    rv = bcmlrd_map_get(unit, L3_EIFt, &map);
    if (SHR_SUCCESS(rv) && map) {
        SHR_IF_ERR_EXIT(bcmtnl_l3_intf_sw_state_cleanup(unit));
    }

    if (bcmtnl_drv_detach(unit) != SHR_E_NONE) {
        return SHR_SYSM_RV_ERROR;
    }
...
{
    if (bcmtnl_init(unit, warm) != SHR_E_NONE) {
        return SHR_SYSM_RV_ERROR;
    }
...
sys_shutdown(shr_sysm_categories_t instance_category,
             ...
             uint32_t *blocking_component)
{
    if (bcmtnl_cleanup(unit) != SHR_E_NONE) {
        return SHR_SYSM_RV_ERROR;
    }
    return SHR_SYSM_RV_DONE;
}
