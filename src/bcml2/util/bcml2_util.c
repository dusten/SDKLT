...
/* BSL Module TBD */
#define BSL_LOG_MODULE BSL_LS_BCML2_TABLE
                         ...
                         int pipe, int index, void *entry_data)
{
    ...
    rsp_entry_wsize  = bcmdrd_pt_entry_wsize(unit, pt_id);
    SHR_IF_ERR_EXIT(
        bcmptm_ltm_ireq_read(unit, BCMPTM_REQ_FLAGS_DONT_USE_CACHE,
                             pt_id, &pt_info, NULL, rsp_entry_wsize, lt_id,
                             entry_data, &rsp_ltid, &rsp_flags));
 exit:
    SHR_FUNC_EXIT();
}
                          ...
                          int pipe, int index, void *entry_data)
{
    ...
    pt_info.tbl_inst = pipe;
    SHR_IF_ERR_EXIT(
        bcmptm_ltm_ireq_write(unit, BCMPTM_REQ_FLAGS_NO_FLAGS, pt_id, &pt_info,
                              NULL, entry_data, lt_id, &rsp_ltid, &rsp_flags));
