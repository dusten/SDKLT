...
#define BCMPTM_SER_OVERLAY_WIDE_VIEW_SIZE    16384
...
* sort by depth of PT view,
* For example: depth of narrow view
* is larger than depth of wide view.
*/
typedef enum bcmptm_ser_mode_index_s {
    /*! narrow mode index in sid_array[] of bcmptm_ser_slices_info_t */
    BCMPTM_SER_NARROW_INDEX,
    /*! wide mode index in sid_array[] of bcmptm_ser_slices_info_t */
    BCMPTM_SER_WIDE_INDEX,
    /*! number of mode, last one*/
    BCMPTM_SER_SLICE_MODE_NUM
} bcmptm_ser_mode_index_t;

/*! \brief Mode of slices */
typedef struct bcmptm_ser_slice_mode_s {
    /*! slice depth for narrow and wide view */
    uint16_t slice_depth[BCMPTM_SER_SLICE_MODE_NUM];
    /*! slice_mode */
    bcmptm_rm_slice_mode_t slice_mode;
} bcmptm_ser_slice_mode_t;

/*! \brief Information of slices */
typedef struct bcmptm_ser_slices_info_s {
    /*! SIDs of wide and narrow view which share the same PT. */
    bcmdrd_sid_t sid_array[BCMPTM_SER_SLICE_MODE_NUM];
    /*! number of sids, should not be larger than BCMPTM_SER_SLICE_MODE_NUM */
    uint16_t sid_num;
    /*! number of slice */
    uint8_t slice_num;
    /*! Flags */
    ...
    /*! WAL read thread will update information of slice_mode,
    * and TCAM thread will get information of slice_mode,
    * so this mutex is used to resolve access conflicts.
    */
                                ...
                                int is_overlay_cam, uint32_t *slice_group_id)
{
    ...
    *slice_group_id = new_group_id;
    ...
    /* created in bcmptm_ser_slice_mode_info_init */
    mode_mutex = slices_info_node->slice_mode_mutex;
    ...
    slices_info_node->slice_mode_mutex = mode_mutex;
                           ...
                           uint32_t *slice_group_id)
{
    ...
    TCAM_SLICE_INFO_ITER(unit, slices_info_node, group_id_index) {
        if ((uint32_t)num_sid != slices_info_node->sid_num) {
            continue;
        }
                                 ...
                                 sal_mutex_t *slice_mode_mutex)
{
    ...
    slices_info_node = TCAM_SLICE_INFO(unit, slice_group_id);

    *slice_mode_mutex = slices_info_node->slice_mode_mutex;
exit:
    SHR_FUNC_EXIT();
}
                                 ...
                                 sal_mutex_t *entry_mode_mutex)
{
    ...
    slices_info = OVERLAY_TCAM_SLICE_INFO(unit);
    /* entry mode mutex and slice mode mutex is the same one */
    *entry_mode_mutex = slices_info->slice_mode_mutex;
...
bcmptm_ser_slice_mode_info_init(int unit, bool warm)
{
    ...
    uint32_t alloc_size_bitmap = SHR_BITALLOCSIZE(BCMPTM_SER_OVERLAY_WIDE_VIEW_SIZE);
    int i = 0, overlay_found = 0;
    sal_mutex_t slice_mode_mutex;

    alloc_size = sizeof(bcmptm_ser_slices_info_t) * BCMPTM_SER_TCAM_NUM;

    slice_info = shr_ha_mem_alloc(unit, BCMMGMT_PTM_COMP_ID,
                                  BCMPTM_HA_SUBID_SER_SLICES_INFO, &alloc_size);
    if (slice_info == NULL) {
        LOG_ERROR(BSL_LOG_MODULE,
                  (BSL_META_U(unit,
    ...
    overlay_bitmap = shr_ha_mem_alloc(unit, BCMMGMT_PTM_COMP_ID,
    ...
    if (overlay_bitmap == NULL) {
        ...
        if(SHR_E_NONE != shr_ha_mem_free(unit, slice_info)) {
            LOG_ERROR(BSL_LOG_MODULE,
                      (BSL_META_U(unit,
                                  "Fail to free HA memory space"
                                  " for slices_info.\n")));
        }
        slice_info = NULL;
        return SHR_E_MEMORY;
    ...
    overlay_wide_slice_bitmap[unit] = (SHR_BITDCL *)overlay_bitmap;
    if (!warm) {
        sal_memset(tcam_slice_info[unit], 0, alloc_size);
        sal_memset(overlay_wide_slice_bitmap[unit], 0, alloc_size_bitmap);
    }
    ...
    for (i = 0; i < BCMPTM_SER_TCAM_NUM; i++) {
        ...
        tcam_slice_info[unit][i].slice_mode_mutex = slice_mode_mutex;
    }
    if (overlay_found == 0) {
...
{
    bcmptm_ser_slices_info_t *slices_info_node;
    uint32_t i = 0;

    for (i = 0; i < BCMPTM_SER_TCAM_NUM; i++) {
        slices_info_node = TCAM_SLICE_INFO(unit, i);
        if (slices_info_node->slice_mode_mutex != NULL) {
            sal_mutex_destroy(slices_info_node->slice_mode_mutex);
            slices_info_node->slice_mode_mutex = NULL;
        }
    }
    if (!warm) {
        if (tcam_slice_info[unit] != NULL) {
            if (shr_ha_mem_free(unit, tcam_slice_info[unit]) != SHR_E_NONE) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit,
                                      "Fail to free HA memory space"
                                      " for tcam slices info.\n")));
            }
        }
        tcam_slice_info[unit] = NULL;
        if (overlay_wide_slice_bitmap[unit] != NULL) {
            if (shr_ha_mem_free(unit, overlay_wide_slice_bitmap[unit]) != SHR_E_NONE) {
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit,
                                      "Fail to free HA memory space"
                                      " for overlay entry bitmap..\n")));
...
bcmptm_ser_slice_mode_mutex_get(int unit, bcmdrd_sid_t sid, sal_mutex_t *slice_mode_mutex)
{
    ...
    *slice_mode_mutex = slice_info_ptr->slice_mode_mutex;
    return SHR_E_NONE;
}
