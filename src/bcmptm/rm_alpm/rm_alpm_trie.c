 ...
 * Defines
 */
#define MAX_KEY_LEN_IPV4  RM_ALPM_IPV4_KEY_SIZE
#define MAX_KEY_LEN_IPV6  RM_ALPM_IPV6_KEY_SIZE

#define TRIE_TRAVERSE_STOP(state, rv) \
    {if (state == TRAVERSE_STATE_DONE || rv < 0) {SHR_RETURN_VAL_EXIT(rv);} }
 ...
 * Private variables
 */
static int repart_debug = 0;
static int trie_validate = 0;
static int propagate_debug = 1;
static rm_alpm_trie_t *repart_trie = NULL;

static shr_lmm_hdl_t trie_internal_nodes = NULL;
static shr_lmm_hdl_t bfs_traverse_nodes = NULL;
static shr_lmm_hdl_t all_tries = NULL;

/*******************************************************************************
 * Private Functions
 */
static int trie_split2_insert(uint32_t            mkl,
                              ...
                              int                 child_count);
static int trie_insert(uint32_t            mkl,
                       rm_alpm_trie_node_t *trie,
                       uint32_t            *key,
                       uint32_t            *bpm,
                       uint32_t            key_len,
                       ...
                       rm_alpm_trie_node_t **child,
                       int                 child_count);
static int trie_delete(uint32_t            mkl,
                       rm_alpm_trie_node_t *trie,
                       uint32_t            *key,
                       uint32_t            key_len,
                       rm_alpm_trie_node_t **payload,
                ...
                uint32_t            mkl)
{
    if (trie != NULL) {
        ...
        if (key == NULL) {
        ...
        } else if (mkl == MAX_KEY_LEN_IPV4) {
            LOG_CLI((BSL_META("trie: %p, type %s, skip_addr 0x%x skip_len %d "
                              "count:%d bpm:0x%x Child[0]:%p Child[1]:%p "
                              "key[0x%08x:0x%08x] len[%d]\n"),
                     (void*)trie, (trie->type == PAYLOAD) ? "P" : "I",
                     trie->skip_addr, trie->skip_len,
                     trie->count, trie->bpm, (void*)trie->child[0].child_node,
                     (void*)trie->child[1].child_node,
            ...
            LOG_CLI((BSL_META("trie: %p, type %s, skip_addr 0x%x skip_len %d "
                     ...
                     trie->count, trie->bpm, (void*)trie->child[0].child_node,
                     (void*)trie->child[1].child_node,
                     key[0], key[1], key[2], key[3], key[4], len));
        }
    }
    return SHR_E_NONE;
}
...
static int
trie_bfs_traverse(rm_alpm_trie_node_t *trie,
                  rm_alpm_trie_callback_f cb,
                  void *user_data)
{
    int rv = SHR_E_NONE;
    ...
    SHR_DQ_INIT(&queue);
    enq_node = shr_lmm_alloc(bfs_traverse_nodes);
    enq_node->node = trie;
    ...
    while (!SHR_DQ_EMPTY(&queue)) {
        ...
        shr_lmm_free(bfs_traverse_nodes, deq_node);
    }
    return rv;
}
...
/* Traverse wrapper */
static int
trie_traverse(rm_alpm_trie_node_t             *trie,
              rm_alpm_trie_callback_f         cb,
              void                            *user_data,
              rm_alpm_trie_traverse_order_e_t order)
{
    ...
    case TRAVERSE_INORDER:
        ...
        break;
    case TRAVERSE_BFS:
        rv = trie_bfs_traverse(trie, cb, user_data);
        break;
    default:
        break;
    }
                 ...
                 int                              *merge_count)
{
    ...
    if (curr_trie_type == PAYLOAD) {
        ...
        if (new_ptrie) {
            SHR_ALLOC(new_ptrie_list, sizeof(trie_list_t), "trie list");
...
trie_dump(rm_alpm_trie_node_t     *trie,
          ...
          uint32_t                mkl)
{
    uint32_t key0[RM_ALPM_MAX_KEY_SIZE_WORDS] = {0};
    uint32_t key1[RM_ALPM_MAX_KEY_SIZE_WORDS] = {0};
    uint32_t len0 = 0;
    uint32_t len1 = 0;
 ...
 */
static int
trie_skip_node_free(uint32_t            mkl,
                    rm_alpm_trie_node_t *trie,
                    uint32_t            *key,
                    uint32_t            key_len)
{
    ...
    case FULL_MATCH_AND_MORE:
        ...
        /* based on next bit branch left or right */
        if (trie->child[bit].child_node) {
            rv = trie_skip_node_free(mkl, trie->child[bit].child_node, key,
                                     key_len - lcp - 1);
            if (SHR_SUCCESS(rv)) {
                TRIE_ASSERT_RETURN(trie->type == INTERNAL);
                shr_lmm_free(trie_internal_nodes, trie);
            }
        } else {
 ...
 */
static int
trie_skip_node_alloc(uint32_t            mkl,
                     rm_alpm_trie_node_t **node,
                     uint32_t            *key,
                     uint32_t            *bpm,
                     uint32_t            msb,
                     ...
                     uint32_t            count)
{
    ...
    for (index = BITS2SKIPOFF(lsb); index <= BITS2SKIPOFF(msb); index++) {
        ...
        if (lsbpos == lsb - 1) {
            ...
            skip_node = shr_lmm_alloc(trie_internal_nodes);
        ...
        if ((msbpos - lsbpos + 1) > MAX_SKIP_LEN) {
            ...
            if (skip_node != payload) {
                shr_lmm_free(trie_internal_nodes, skip_node);
                SHR_RETURN_VAL_EXIT(SHR_E_INTERNAL);
            }
        }
               ...
               int                 lcp)
{
    ...
    if (!trie->child[bit].child_node) {
        ...
        /* should be the child */
        rv = trie_skip_node_alloc(mkl, &node, key, bpm,
        ...
        rv = trie_insert(mkl, trie->child[bit].child_node,
               ...
               int                 lcp)
{
    ...
    node = shr_lmm_alloc(trie_internal_nodes);
    SHR_NULL_CHECK(node, SHR_E_MEMORY);

    CLONE_TRIE_NODE(node, trie);

    rv = trie_skip_node_alloc(mkl, &newchild, key, bpm,
                              key_len - lcp - 1,
                              key_len - lcp - 1,
                              payload, child_count);
    if (SHR_SUCCESS(rv)) {
        ...
        *child = node;
        ...
        LOG_CLI((BSL_META("\n Error on trie skip node allocation [%d]!!!!\n"),
                 rv));
        shr_lmm_free(trie_internal_nodes, node);
    }
    SHR_RETURN_VAL_EXIT(rv);
exit:
    SHR_FUNC_EXIT();
            ...
            int                 child_count)
{
    ...
    *child = NULL;
    ...
    switch (trie_match_type(key_len, lcp, trie->skip_len)) {
    case EXACT_MATCH:
        rv = trie_insert_em(mkl, trie, key, bpm, key_len,
                            payload, child, child_count, lcp);
        break;
    case FULL_MATCH_AND_MORE:
        rv = trie_insert_fm(mkl, trie, key, bpm, key_len,
                            payload, child, child_count, lcp);
        break;
    case OUT_MATCH:
    ...
    case PARTIAL_MATCH:
    default:
        rv = trie_insert_pm(mkl, trie, key, bpm, key_len,
                            payload, child, child_count, lcp);
        break;
    }
 ...
 */
static int
trie_fuse_child(rm_alpm_trie_node_t *trie, int bit)
{
               ...
               int                 lcp)
{
    ...
    rv = trie_delete(mkl, trie->child[bit].child_node,
            ...
            rm_alpm_trie_node_t **child)
{
    ...
    *child = NULL;
    ...
    switch (trie_match_type(key_len, lcp, trie->skip_len)) {
    case FULL_MATCH_AND_MORE:
        rv = trie_delete_fm(mkl, trie, key, key_len, payload, child, lcp);
        break;
    case EXACT_MATCH:
        rv = trie_delete_em(mkl, trie, key, key_len, payload, child, lcp);
        break;
    default:
        rv = SHR_E_NOT_FOUND;
    }
 ...
 */
    uint32_t split_type = SPLIT_FURTHER;
    ...
    if (trie->count != max_count && trie->count <= max_split_count) {
        if (length > max_split_len) {
            ...
            uint32_t half_count = (max_count + 1) >> 1;
             if (TRIE_SPLIT_STATE_PAYLOAD_SPLIT == state) {
                if (child == NULL) {
                    split_type = SPLIT_GOOD;
                } else if (trie->type == PAYLOAD) {
                    if (trie->count >= half_count && child->count < half_count &&
                            trie->count <= (((max_count << 1) + 2) / 3)) {
                        split_type = SPLIT_GOOD;
 ...
 */
static int
trie_split_middle_point(uint32_t            mkl,
                        rm_alpm_trie_node_t *trie,
                        uint32_t            *pivot,
                        uint32_t            *length,
                        ...
                        uint32_t            *bpm)
{
    ...
    *split_node = shr_lmm_alloc(trie_internal_nodes);
           ...
           const int             max_split_count)
{
    ...
    if (split_type == SPLIT_FURTHER) {
        ...
        rv = trie_split(mkl, trie->child[bit].child_node,
                        pivot, length,
        ...
        *state = TRIE_SPLIT_STATE_PRUNE_NODES;
        ...
        if (trie->skip_len == 0) {
            ...
            *state = TRIE_SPLIT_STATE_PRUNE_NODES;
            ...
            rv = trie_split_middle_point(mkl, trie,
                                         pivot, length,
                                         split_count, split_node,
                                         max_split_len, bpm);
            *state = TRIE_SPLIT_STATE_PRUNE_NODES;
    ...
    case TRIE_SPLIT_STATE_PAYLOAD_SPLIT_DONE:
        if (trie->type == PAYLOAD) {
            ...
            *split_count = trie->count;
            ...
            TRIE_ASSERT_RETURN(*length >= trie->skip_len + 1);
            *length -= (trie->skip_len + 1);
        }
        break;
 ...
 */
static int trie_merge(uint32_t            mkl,
                      ...
                      rm_alpm_trie_node_t **new_parent)
{
    ...
    uint32_t bpm[RM_ALPM_MAX_KEY_SIZE_WORDS] = {0};
    uint32_t child_pivot[RM_ALPM_MAX_KEY_SIZE_WORDS] = {0};
    ...
    if (lcp == trie->skip_len) {
        ...
        if (key_len > lcp) {
            ...
            if (trie->child[bit].child_node) {
                ...
                rv = trie_split2(mkl, trie->child[bit].child_node,
                                 key, key_len - lcp - 1,
 ...
 */
        if ((trie->bpm & BITMASK(trie->skip_len - len)) == 0) {
            ...
            if (SHR_E_LIMIT == rv) rv = SHR_E_NONE;
        }
 ...
 */
static void
trie_destroy(rm_alpm_trie_t *trie)
{
    shr_lmm_free(all_tries, trie);
}
 ...
 */
int bcmptm_rm_alpm_trie_traverse(rm_alpm_trie_t                  *trie,
                                 rm_alpm_trie_callback_f         cb,
                                 void                            *user_data,
                                 rm_alpm_trie_traverse_order_e_t order)
{
    ...
    if (trie == NULL) {
        ...
        return trie_traverse(trie->trie, cb, user_data, order);
    }
}
 ...
 */
int bcmptm_rm_alpm_trie_traverse2(rm_alpm_trie_t                  *trie,
                                  rm_alpm_trie_callback_ext_f     cb,
                                  void                            *user_data,
                                  rm_alpm_trie_traverse_order_e_t order)
{
    ...
    if (trie == NULL) {
        ...
        return trie_traverse2(trie->trie, cb, user_data, order, &state);
    }
}
/*
 * Function:
...
int bcmptm_rm_alpm_trie_dump(rm_alpm_trie_t          *trie,
                             ...
                             void                    *user_data)
{
    uint32_t key[RM_ALPM_MAX_KEY_SIZE_WORDS] = {0};
    uint32_t len = 0;
    ...
    if (trie->trie) {
        ...
        if (lpm_pfx) {
            sal_memset(lpm_pfx, 0,
                       sizeof(uint32_t) * RM_ALPM_MAX_KEY_SIZE_WORDS);
        }
 ...
 */
int bcmptm_rm_alpm_trie_insert(rm_alpm_trie_t      *trie,
                               uint32_t            *key,
                               uint32_t            *bpm,
                               uint32_t            length,
                               rm_alpm_trie_node_t *payload)
{
    ...
    if (trie->trie == NULL) {
        rv = trie_skip_node_alloc(trie->mkl,
                                  &trie->trie, key, bpm, length, length,
                                  payload, 1);
    } else {
        rv = trie_insert(trie->mkl,
 ...
 */
int bcmptm_rm_alpm_trie_delete(rm_alpm_trie_t      *trie,
                               uint32_t            *key,
                               uint32_t            length,
                               rm_alpm_trie_node_t **payload)
{
 ...
 */
int bcmptm_rm_alpm_trie_split(rm_alpm_trie_t      *trie,
                              const uint32_t      max_split_len,
                              uint32_t            *pivot,
                              uint32_t            *length,
    ...
    if (trie->trie) {
        ...
        if (SHR_SUCCESS(rv) && (TRIE_SPLIT_STATE_DONE == state)) {
            ...
            if (trie->trie == NULL) {
                ...
                bcmptm_rm_alpm_trie_create(MAX_KEY_LEN_IPV4, &c1);
                bcmptm_rm_alpm_trie_create(MAX_KEY_LEN_IPV4, &c2);
                c1->trie = child;
             ...
             */
            rv = trie_skip_node_alloc(trie->mkl, &node, pivot, NULL,
                                      *length, *length,
                                      child, child->count);
 ...
 *     merge or fuse the child trie with parent trie.
 */
int bcmptm_rm_alpm_trie_merge(rm_alpm_trie_t      *parent_trie,
                              rm_alpm_trie_node_t *child_trie,
...
int bcmptm_rm_alpm_trie_split2(rm_alpm_trie_t      *trie,
                               uint32_t            *key,
                               uint32_t            key_len,
                               uint32_t            *pivot,
                               uint32_t            *pivot_len,
                               rm_alpm_trie_node_t **split_trie_root)
{
    ...
    *pivot_len = 0;
    ...
    if (trie->trie) {
        rm_alpm_trie_node_t *payload;
        sal_memset(pivot, 0, sizeof(uint32_t) * BITS2WORDS(trie->mkl));
        payload = shr_lmm_alloc(trie_internal_nodes);
        ...
        if (SHR_SUCCESS(rv)) {
            ...
            shr_lmm_free(trie_internal_nodes, payload);
        ...
        child = NULL;
        rv = trie_split2(trie->mkl, trie->trie, key, key_len, pivot, pivot_len,
 ...
 */
int bcmptm_rm_alpm_trie_create(uint32_t mkl, rm_alpm_trie_t **ptrie)
{
    rm_alpm_trie_t *trie = NULL;
    SHR_FUNC_ENTER(BSL_UNIT_UNKNOWN);

    trie = shr_lmm_alloc(all_tries);
    SHR_NULL_CHECK(trie, SHR_E_MEMORY);

    sal_memset(trie, 0, sizeof(rm_alpm_trie_t));

    if (mkl == MAX_KEY_LEN_IPV4) {
        trie->v6_key = FALSE;
    } else if (mkl == MAX_KEY_LEN_IPV6) {
        trie->v6_key = TRUE;
    } else {
        shr_lmm_free(all_tries, trie);
        SHR_RETURN_VAL_EXIT(SHR_E_PARAM);
 ...
 */
int bcmptm_rm_alpm_trie_destroy(rm_alpm_trie_t *trie)
{
    trie_destroy(trie);
    return SHR_E_NONE;
}
                                 ...
                                 shr_lmm_hdl_t *clone_hdl)
{
    ...
    rv = bcmptm_rm_alpm_trie_create(trie_src->mkl, trie_dst);
    if (SHR_FAILURE(rv)) {
        SHR_RETURN_VAL_EXIT(rv);
    }
    ...
    if (SHR_FAILURE(rv)) {
        bcmptm_rm_alpm_trie_clone_destroy(*trie_dst, *clone_hdl);
        *trie_dst = NULL;
    }
    SHR_RETURN_VAL_EXIT(rv);
...
void
bcmptm_rm_alpm_trie_clone_destroy(rm_alpm_trie_t *trie_dst,
                                  shr_lmm_hdl_t clone_hdl)
{

    if (trie_dst && clone_hdl) {
        shr_lmm_delete(clone_hdl);
        trie_destroy(trie_dst);
    }
}
...
int bcmptm_rm_alpm_trie_init(bool multi_thread)
{
    int rv;
    LMEM_MGR_INIT(rm_alpm_trie_node_t,
                  trie_internal_nodes,
...
void
bcmptm_rm_alpm_trie_cleanup(void)
{
    if (trie_internal_nodes) {
        shr_lmm_delete(trie_internal_nodes);
        trie_internal_nodes = NULL;
    }
    if (bfs_traverse_nodes) {
        shr_lmm_delete(bfs_traverse_nodes);
        bfs_traverse_nodes = NULL;
    }
    if (all_tries) {
        shr_lmm_delete(all_tries);
        all_tries = NULL;
    }
}
