 ...
 * Private variables
 */
static bool rm_hash_init_warm[BCMDRD_CONFIG_MAX_UNITS];
...
my_init(int unit, bool warm)
{
    ...
    tmp_rv = bcmptm_rm_hash_init(unit);
                      ...
                      uint32_t *blocking_component)
{
    ...
    if (!rm_hash_init_warm[unit]) { /* WB_NOT_YET_SUPPORTED */
