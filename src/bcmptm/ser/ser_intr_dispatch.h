...
typedef struct bcmptm_ser_intr_msg_q_info_s {
    int          unit;         /*!< Device ID */
    uint32_t     reg_id;       /*!< Id of CMIC_CMC_IRQ_STATx */
    uint32_t     bit_offset;   /*!< Bit offset of CMIC_CMC_IRQ_STATx */
    uint32_t     intr_param;   /*!< Now, only used for MMU "interrupt number | instance" */
} bcmptm_ser_intr_msg_q_info_t;

/*!
 * \brief create interrupt dispatching message queue.
 * \n If message queue had been created, old queue is not destoryed,
 * \n and new queue is not created.
 *
...
/*!
 * \brief Destroy interrupt dispatching message queue.
 *
 * \param [in] unit Logical device id
 */
extern void
...
/*!
 * \brief When there is SER interrupt from non-MMU blocks, interrupt dispatching thread will
 * \n call this callback routine, and then it will post messages to SERC thread.
...
/*!
 * \brief When there is SER interrupt from MMU block, MMU interrupt dispatching thread will
 * \n call this callback routine, and then it will post messages to SERC thread.
 * \n When SERC thread receives the messages, it will correct SER error.
 *
 * \param [in] unit Logical device id
 * \param [in] intr_num  Logical interrupt source id of MMU | inst
 *
 * \retval none
 */
extern void
bcmptm_ser_mmu_notify(int unit, uint32_t intr_num);

/*!
 * \brief try to recieve SER event message
