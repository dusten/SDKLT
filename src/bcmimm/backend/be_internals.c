...
#define ELEM_CHUNK_SIZE    2000

/*!
 * The maximal index for a free block. Since each block associated with
 * unique sub module ID and there maximum 255 ID's we allow a single size
 * of elements to use up to half of the sub module ID range.
 */
#define MAX_BLK_IDX     127

/*!
 * \brief Helper array mapps between HA block sub ID to physical memory.
 */
static element_list_head_t *fix_element_blk[BCMDRD_CONFIG_MAX_UNITS][256];
                         ...
                         bcmimm_free_list_t **f_list)
{
    ...
    SHR_FUNC_ENTER(unit);
    SHR_ALLOC(new_list, sizeof(bcmimm_free_list_t), "IMM_BE_EL");
    SHR_NULL_CHECK(new_list, SHR_E_MEMORY);
    new_list->lock = sal_mutex_create("IMM_BE");
 ...
 * \param [in] array_idx Indicating the location of this block in the chain
 * of free block. All blocks, serving the same data size, are linked together
 * as well as are being pointed (normal pointer) by an array of the blocks
 * of the chain.
                          ...
                          element_list_head_t *first_blk)
{
    ...
    for (idx = 1; idx < ELEM_CHUNK_SIZE; idx++) {
 ...
 *
 * This function is called to allocate a free block (HA) containing elemens
                                        ...
                                        bcmimm_free_list_t **list)
{
    ...
    uint8_t sub = bcmimm_available_subs[unit]++;
    bcmimm_free_list_t *new_list;

    /* Need to allocate a new list */
    /*
     * Figure out the sub to use. Use bcmimm_available_subs.
     */
    ...
    new_list = sal_alloc(sizeof(*new_list), "IMM_BE_EL");
    if (new_list == NULL) {
        return NULL;
    }
 ...
 *
 * \param [in] unit This is the unit associated with this block elements.
 * \param [in] elem_fr_lst This is the element free list which manages
                            ...
                            bcmimm_free_list_t *f_list)
{
    ...
    len = ELEM_CHUNK_SIZE * f_list->data_len +
           sizeof(element_list_head_t);
    blk = shr_ha_mem_alloc(unit,
                           bcmimm_be_comp_id,
                           sub,
    ...
    fix_element_blk[unit][sub] = blk;
    free_blk_init(blk, f_list->master_blk);
 ...
 * the indexes in the \c idx_array.
 *
 ...
 * \return None
 */
static void quicksort(uint32_t *idx_array,
    ...
    if (start_idx < end_idx)
    {
        ...
        while (index1 < index2)
        {
            while((index1 < end_idx) &&
               (ref_array[idx_array[index1]] <= ref_array[idx_array[pivot]])) {
                index1++;
            }
            while (ref_array[idx_array[index2]] > ref_array[idx_array[pivot]]) {
                index2--;
            }
         ...
         */
        tmp = idx_array[pivot];
        idx_array[pivot] = idx_array[index2];
        idx_array[index2] = tmp;

        /* Recursive call for quick sort, with partiontioning */
        quicksort(idx_array, ref_array, start_idx, index2-1);
        quicksort(idx_array, ref_array, index2+1, end_idx);
    }
}
...
static int fid_sort(bcmimm_tbl_info_t *t_info, uint32_t *fid, size_t count)
{
    ...
    quicksort(t_info->sorted_fid, fid, 0, (int)(count - 1));
    return SHR_E_NONE;
}
 ...
 */
int bcmimm_free_lists_retrieve(int unit, uint8_t sub)
{
    element_list_head_t *f_blk;
                                         ...
                                         bcmimm_free_list_t **list)
{
    ...
    /* If not found any suitable list */
    if (rv == NULL) {
                           ...
                           const bcmimm_be_fields_upd_t *in)
{
    ...
    while (ent_ptr && (count < in->count)) {
        ...
        if (in->del[in_idx]) {
            SHR_IF_ERR_EXIT(
                field_del(t_info, in, in_idx, &ent_ptr));
        } else {
            SHR_IF_ERR_EXIT(
                                ...
                                uint32_t *start_ptr )
{
    ...
    if (BCMIMM_IS_NULL_PTR(*start_ptr )) {
        /*
         * If the pointer is done it means that there is nothing more to do
        ...
        ent = base + t_info->tbl->ent_len * start_idx;
        ptr = (uint32_t *)(ent + ptr_offset);
    } else {
